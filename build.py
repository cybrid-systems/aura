...
def cmd_lint():
    ...
    hc3043_script = COVERAGE_CHECKS / "check_hot_contract_soft_observe_3043.py"
    if not hc3043_script.exists():
        fail(f"missing {hc3043_script}")
        return 1
    r = run([sys.executable, str(hc3043_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3067: stale record_dependency reject schedules deferred
    # hybrid_node_cascade_. Extends test_dep_graph_hybrid_cascade (#81967).
    hdc3067_script = COVERAGE_CHECKS / "check_hybrid_deferred_cascade_3067.py"
    if not hdc3067_script.exists():
        fail(f"missing {hdc3067_script}")
        return 1
    r = run([sys.executable, str(hdc3067_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3082: mid/nested MutationBoundary occurrence is
    # provisional — no durable persist; query:type in-flight until
    # outermost commit. Extends persist-rehydrate (#81967).
    nop3082_script = COVERAGE_CHECKS / "check_nested_occurrence_provisional_3082.py"
    if not nop3082_script.exists():
        fail(f"missing {nop3082_script}")
        return 1
    r = run([sys.executable, str(nop3082_script)], cwd=ROOT)
    if r != 0:
        fail(
            "Issue #3082 nested occurrence provisional linter failed — run python3 scripts/coverage/checks/check_nested_occurrence_provisional_3082.py"
        )
        return r
    ...
    if r != 0:
        ...
        return r
    # Issue #3230: production query:*-stable hard-rejects restamp-lag
    # before make_ref_layout / stamp-green. Soft observe; budget=0 quiet.
    # Extends test_hygiene_mutate_closed_loop; no docs/design / invent.
    qsrl3230_script = COVERAGE_CHECKS / "check_query_stable_restamp_lag_hard_reject_3230.py"
    if not qsrl3230_script.exists():
        fail(f"missing {qsrl3230_script}")
        return 1
    r = run([sys.executable, str(qsrl3230_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    scnas3199_script = COVERAGE_CHECKS / "check_shape_compact_no_all_shards_lock_3199.py"
    if not scnas3199_script.exists():
        fail(f"missing {scnas3199_script}")
        return 1
    r = run([sys.executable, str(scnas3199_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3251: unified deny-class on spawn/join/send fail hashes.
    # Extends test_orch_obs_facade + quota/BP/schedule suites (#81967);
    # no docs/design/ (#1655).
    adc_script = COVERAGE_CHECKS / "check_agent_deny_class_3251.py"
    if not adc_script.exists():
        fail(f"missing {adc_script}")
        return 1
    r = run([sys.executable, str(adc_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3253: repair SOLVED residual dirty drain before green stamp.
    # Extends test_solve_delta_unresolved_export.cpp (#81967); no
    # docs/design/ (#1655).
    rsr_script = COVERAGE_CHECKS / "check_repair_solved_residual_3253.py"
    if not rsr_script.exists():
        fail(f"missing {rsr_script}")
        return 1
    r = run([sys.executable, str(rsr_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3262: GC safepoint restamp after dual shared_locks +
    # audit acquire-load of linear_ownership_gc_violations_prevented.
    # Extends test_linear_provenance_steal_gc_closed_loop.cpp +
    # test_linear_batch.cpp (#81967); no docs/design/ (#1655).
    gsr_script = COVERAGE_CHECKS / "check_gc_safepoint_restamp_lock_3262.py"
    if not gsr_script.exists():
        fail(f"missing {gsr_script}")
        return 1
    r = run([sys.executable, str(gsr_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3017: densify incomplete-remap residual — value-only prep
    # is not safe cover; expand allocate-bypass lint to FFI/agent/scratch.
    # Extends test_moving_densify_fail_closed (#81967); no docs/design/ (#1655).
    mir_script = COVERAGE_CHECKS / "check_moving_incomplete_remap_3017.py"
    if not mir_script.exists():
        fail(f"missing {mir_script}")
        return 1
    r = run([sys.executable, str(mir_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    # (#81967); no docs/design/ (#1655).
    ecap_script = COVERAGE_CHECKS / "check_envframe_closure_apply_protocol_3021.py"
    if not ecap_script.exists():
        fail(f"missing {ecap_script}")
        return 1
    r = run([sys.executable, str(ecap_script)], cwd=ROOT)
    if r != 0:
        fail(
    ...
    if r != 0:
        ...
        return r
    # Issue #3243: production missing/overlap region_keys stay Serialized
    # with an observable missing-keys signal (no false concurrent).
    # Extends test_parallel_intend_pure_contract.cpp (#81967); no
    # docs/design/ (#1655).
    prk3243_script = COVERAGE_CHECKS / "check_parallel_region_key_missing_3243.py"
    if not prk3243_script.exists():
        fail(f"missing {prk3243_script}")
        return 1
    r = run([sys.executable, str(prk3243_script)], cwd=ROOT)
    if r != 0:
        fail(
...
def cmd_moving_incomplete_remap_3017_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("moving incomplete-remap residual (#3017) coverage clean")
    return 0


def cmd_moving_incomplete_remap_3017():
    """Issue #3017: densify incomplete-remap residual (value-only not cover).

    Value-only register_external_root_for_densify is observability only.
    Production hard still pre-move rejects uncovered externals + sticky-off.
    Soft / Off stays a single atomic load. Lint expands to FFI/agent/scratch.
    """
...
def cmd_envframe_closure_apply_protocol_3021_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("EnvFrame/Closure apply protocol (#3021) coverage clean")
    return 0


def cmd_envframe_closure_apply_protocol_3021():
    """Issue #3021: unguarded EnvFrame/Closure apply is a forced protocol.

    Freed/tombstone at apply/use rejects (scan_skip_freed semantics).
    Soft extra cost is one lifetime_version load. No second model.
    """
...
def cmd_gc_safepoint_restamp_lock_3262_coverage():
...
def cmd_repair_solved_residual_3253_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("repair SOLVED residual (#3253) coverage clean")
    return 0
...
def cmd_agent_deny_class_3251_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("agent deny-class (#3251) coverage clean")
    return 0
...
def cmd_hybrid_deferred_cascade_3067_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("hybrid deferred cascade (#3067) coverage clean")
    return 0


def cmd_hybrid_deferred_cascade_3067():
    """Issue #3067: stale record_dependency reject schedules hybrid cascade.

    Soft/clean: zero extra when no reject.
    """
    print(f"{B}=== hybrid deferred cascade (#3067) ==={N}")
...
def cmd_nested_occurrence_provisional_3082_coverage():
    ...
    if r.returncode != 0:
        ...
        return 1
    ok("nested occurrence provisional (#3082) coverage clean")
    return 0


def cmd_nested_occurrence_provisional_3082():
    """Issue #3082: mid/nested MutationBoundary marks occurrence provisional.

    Nested success never durable-persists. query:type stays in-flight
    until outermost persist. Soft empty / no nested: zero extra persist.
    """
    print(f"{B}=== nested occurrence provisional (#3082) ==={N}")
    return cmd_nested_occurrence_provisional_3082_coverage()
...
def main():
    ...
    commands = {
        ...
        "nested-occurrence-provisional-3082": cmd_nested_occurrence_provisional_3082,
        "nested-occurrence-provisional-3082-coverage": cmd_nested_occurrence_provisional_3082_coverage,
