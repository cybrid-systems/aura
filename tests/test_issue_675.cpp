...
static std::string hash_string(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format("(hash-ref (query:ci-reproducibility-stats) '{}')", key));
    if (!r || !aura::compiler::types::is_string(*r)) return {};
...
int main() {
    ...
    // AC2: 5 fields
    {
        ...
        auto repro = cs.eval("(hash-ref (query:ci-reproducibility-stats) 'reproducible-flags-active)");
...
#endif
        CHECK(hash_bool(cs, "reproducible-flags-active"), "reproducible-flags-active #t with epoch set");
        CHECK(hash_int(cs, "source-date-epoch") == 1704067200, "source-date-epoch == 1704067200");
#if defined(_WIN32)
        if (saved.empty()) _putenv_s("SOURCE_DATE_EPOCH", "");
        else _putenv_s("SOURCE_DATE_EPOCH", saved.c_str());
#else
        if (saved.empty()) unsetenv("SOURCE_DATE_EPOCH");
        else setenv("SOURCE_DATE_EPOCH", saved.c_str(), 1);
#endif
    }
