...
static void ac5_gate() {
    ...
    CHECK(build.find("cmd_gc_closures_mtx_flush_sweep_coverage") != std::string::npos,
          "AC5: coverage cmd");
    CHECK(cmake.find("test_gc_closures_mtx_flush_sweep") != std::string::npos, "AC5: cmake test");
    CHECK(!script.empty() && script.find("2473") != std::string::npos, "AC5: check script exists");
}

} // namespace

int run_test_gc_closures_mtx_flush_sweep() {
...
int main() {
    return run_test_gc_closures_mtx_flush_sweep();
}
#endif
