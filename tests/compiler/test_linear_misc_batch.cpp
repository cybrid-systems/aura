...
// test_linear_misc_batch (S3 renamed members)
...
int main() {
    ...
    if (run_test_linear_enforce_boundary_align() != 0 || g_failed != 0) {
        ++members_failed;
        std::println(
            "FAIL member test_linear_enforce_boundary_align (checks: {} passed, {} failed)",
            g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_enforce_production_defaults() != 0 || g_failed != 0) {
        ++members_failed;
        std::println(
            "FAIL member test_linear_enforce_production_defaults (checks: {} passed, {} failed)",
            g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_enforce_strict() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_enforce_strict (checks: {} passed, {} failed)",
                     g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_enforce_strict_default() != 0 || g_failed != 0) {
        ++members_failed;
        std::println(
            "FAIL member test_linear_enforce_strict_default (checks: {} passed, {} failed)",
            g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_escape_commit_hardblock() != 0 || g_failed != 0) {
        ++members_failed;
        std::println(
            "FAIL member test_linear_escape_commit_hardblock (checks: {} passed, {} failed)",
            g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_force_unified() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_force_unified (checks: {} passed, {} failed)",
                     g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_gc_window() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_gc_window (checks: {} passed, {} failed)", g_passed,
                     g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_partial_revalidate() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_partial_revalidate (checks: {} passed, {} failed)",
                     g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_synth_boundary_authority() != 0 || g_failed != 0) {
        ++members_failed;
        std::println(
            "FAIL member test_linear_synth_boundary_authority (checks: {} passed, {} failed)",
            g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_synth_violation() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_synth_violation (checks: {} passed, {} failed)",
                     g_passed, g_failed);
    } else {
        ++members_passed;
    ...
    if (run_test_linear_three_layer_wire() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL member test_linear_three_layer_wire (checks: {} passed, {} failed)",
                     g_passed, g_failed);
    } else {
        ++members_passed;
