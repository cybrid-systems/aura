...
int run_test_isolation_stamp_resolve() {
    ...
    // ── AC7: query:tenant-isolation-stats primitive returns a hash ──
    {
        ...
        auto size_r = cs.eval(R"((hash-length (engine:metrics "query:tenant-isolation-stats")))");
...
int main() {
    return run_test_isolation_stamp_resolve();
}
#endif
