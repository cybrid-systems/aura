...
static void ac5_source_gate() {
    ...
    CHECK(cmake.find("test_try_catch_bind") != std::string::npos, "AC5: cmake");
    const auto build = read_file("build.py");
    CHECK(!read_file("scripts/coverage/manifests/2567.json").empty(),
...
int main() {
    return run_test_try_catch_bind();
}
#endif
