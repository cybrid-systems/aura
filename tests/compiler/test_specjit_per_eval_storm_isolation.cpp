...
static void ac5_source_and_gate() {
    ...
    const auto script = read_file("scripts/coverage/manifests/2370.json");
...
int run_test_specjit_per_eval_storm_isolation() {
    std::println("test_specjit_per_eval_storm_isolation");
    ac1_global_soft();
...
int main() {
    return run_test_specjit_per_eval_storm_isolation();
}
#endif
