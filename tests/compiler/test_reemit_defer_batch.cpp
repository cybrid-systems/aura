...
extern int run_test_reemit_production_provider_wired();

int main() {
    using aura::test::g_failed;
    using aura::test::g_passed;
    int members_failed = 0, members_passed = 0;
    std::println("=== test_reemit_defer_batch (3 members) ===");
    ...
    std::println("\n──── test_reemit_production_provider_wired_3373 ────");
    g_passed = 0;
    g_failed = 0;
    if (run_test_reemit_production_provider_wired() != 0 || g_failed != 0) {
        ++members_failed;
        std::println("FAIL test_reemit_production_provider_wired_3373");
    } else {
        ++members_passed;
        std::println("OK test_reemit_production_provider_wired_3373");
    }
