...
static void ac5_gate() {
    ...
    CHECK(build.find("cmd_aura_jit_unused_fn_lock_coverage") != std::string::npos,
          "AC5: coverage cmd");
    CHECK(cmake.find("test_aura_jit_unused_fn_lock") != std::string::npos, "AC5: cmake test");
    CHECK(!script.empty() && script.find("2475") != std::string::npos, "AC5: check script exists");
}

} // namespace

int run_test_aura_jit_unused_fn_lock() {
...
int main() {
    return run_test_aura_jit_unused_fn_lock();
}
#endif
