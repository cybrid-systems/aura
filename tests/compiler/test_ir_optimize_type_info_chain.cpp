...
static void ac5_gate() {
    ...
    CHECK(build.find("cmd_ir_optimize_type_info_chain_coverage") != std::string::npos,
          "AC5: coverage cmd");
    CHECK(cmake.find("test_ir_optimize_type_info_chain") != std::string::npos, "AC5: cmake test");
    CHECK(!script.empty() && script.find("2471") != std::string::npos, "AC5: check script exists");
}

} // namespace

int run_test_ir_optimize_type_info_chain() {
...
int main() {
    return run_test_ir_optimize_type_info_chain();
}
#endif
