...
CMDS = [
    ...
    cmd('(require "std/orchestrator" all:)(agent:spawn "plan" (lambda (t) (string-append "plan:" t)))(agent:spawn "code" (lambda (p) (string-append "code:" p)))"t1:ok"'),
    # t2: agent:ask same-session
    cmd('(require "std/orchestrator" all:)(string-append "t2:" (agent:ask "plan" "fib"))'),
    # t3: pipeline across agents
    cmd('(require "std/orchestrator" all:)(define p (agent:ask "plan" "fib"))(define c (agent:ask "code" p))(string-append "t3:" c)'),
    # t4: create cross-session sessions
    cmd('(session:create "sa")(session:create "sb")(string-append "t4:" (if (session-active? "sa") "sa-ok" "sa-fail") "," (if (session-active? "sb") "sb-ok" "sb-fail"))'),
    # t5: agent list
    ...
    cmd('(require "std/orchestrator" all:)(agent:spawn "helper" (lambda (x) (string-append "h:" x)))"t7:ok"', session="sa"),
    cmd('(require "std/orchestrator" all:)(string-append "t7:" (agent:ask "helper" "task"))', session="sb"),
    # t8: lifecycle
    cmd('(require "std/orchestrator" all:)(agent:status "helper")', session="sa"),
    cmd('(require "std/orchestrator" all:)(agent:stop "helper")(agent:status "helper")', session="sa"),
    cmd('(require "std/orchestrator" all:)(agent:restart "helper" (lambda (x) (string-append "v2:" x)))(agent:ask "helper" "test")', session="sa"),
    # done
    cmd('"t9:done"'),
]
...
def main():
    ...
    payload = "\n".join(CMDS)
    t0 = time.time()
    r = subprocess.run([AURA, "--serve-async"], input=payload,
