// Issue #381/#406/#506/#571 (#1978 renamed): issue# moved from filename to header.
...
static std::int64_t pass_stats(CompilerService& cs) {
    auto r = cs.eval("(engine:metrics \"query:pass-contracts-stats\")");
    if (!r || !is_int(*r))
        return 0;
    return as_int(*r);
}
...
static void run_matrix(CompilerService& cs) {
    ...
    std::println("  jit ok={} stats: {} -> {}", jit.has_value(), stats3a, stats3b);
