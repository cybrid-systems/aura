...
//   - AC2: (engine:metrics \"query:type-propagation-stats\") returns integer sum
...
bool test_query_type_propagation_stats() {
    std::println(
        "\n--- AC2: (engine:metrics \"query:type-propagation-stats\") returns integer ---");
    CompilerService cs;
    (void)cs.eval("(set-code \"(define a 1)\")");
    (void)cs.eval("(eval-current)");
    auto r = cs.eval("(engine:metrics \"query:type-propagation-stats\")");
    CHECK(r.has_value(), "(engine:metrics \"query:type-propagation-stats\") returns");
    CHECK(aura::compiler::types::is_int(*r),
          "(engine:metrics \"query:type-propagation-stats\") is integer");
    if (r && aura::compiler::types::is_int(*r)) {
        const auto v = aura::compiler::types::as_int(*r);
        std::println("  query:type-propagation-stats = {}", v);
        CHECK(v >= 0, "(engine:metrics \"query:type-propagation-stats\") >= 0 (sum of 4 counters)");
    }
    return true;
}
...
bool test_gc_heap_with_type_propagation() {
    ...
    auto r2 = cs.eval("(engine:metrics \"query:type-propagation-stats\")");
    CHECK(r2.has_value() && aura::compiler::types::is_int(*r2),
          "(engine:metrics \"query:type-propagation-stats\") after gc-heap");
    return true;
}
...
bool test_regression_existing_primitives() {
    ...
    (void)cs.eval("(set-code \"(define a 1) (define b 2)\")");
    (void)cs.eval("(eval-current)");
    auto r1 = cs.eval("(engine:metrics \"query:type-propagation-stats\")");
    CHECK(r1.has_value() && aura::compiler::types::is_int(*r1),
          "(engine:metrics \"query:type-propagation-stats\") (new for #305)");
