...
bool test_lookup_count_increments() {
    ...
    if (!r1) {
        std::println("  FAIL: define _ failed");
        ++g_failed;
        return false;
    }
...
bool test_per_symbol_less_than_ancestor() {
    ...
    cs.eval("(set-code \"(define (f) (let ((x 1) (y 2) (z 3) (w 4) (v 5)) (+ x 1))) (define h "
            "(compile:per-symbol-dirty-stats \\\"x\\\"))\")");
    cs.eval("(eval-current)");
    ...
    std::println("  per-symbol-affected-count={}, ancestor-affected-count={}", per_symbol,
                 ancestor);
    ...
    CHECK(per_symbol > 0, "per-symbol-affected-count > 0 (Variable(x) exists in body)");
    CHECK(ancestor > 0, "ancestor-affected-count > 0 (def node x found)");
    CHECK(per_symbol <= ancestor, "per-symbol <= ancestor (per-symbol is subset of ancestor)");
    return true;
}
...
bool test_unbound_sym_returns_sensible() {
    ...
    cs.eval("(set-code \"(define (g) 42) (define h (compile:per-symbol-dirty-stats "
            "\\\"missing-sym-xyz\\\"))\")");
    cs.eval("(eval-current)");
    auto rps = cs.eval("(hash-ref h \"per-symbol-affected-count\")");
    auto rac = cs.eval("(hash-ref h \"ancestor-affected-count\")");
    auto rrb = cs.eval("(hash-ref h \"reduction-ratio-bp\")");
    if (!rps || !rac || !rrb || !aura::compiler::types::is_int(*rps) ||
        !aura::compiler::types::is_int(*rac) || !aura::compiler::types::is_int(*rrb)) {
        std::println("  FAIL: could not read counts");
        ++g_failed;
        return false;
    }
    ...
    std::println("  unbound sym: per-symbol={}, ancestor={}, ratio={}", per_symbol, ancestor,
                 ratio);
...
bool test_reduction_ratio_computes_correctly() {
    ...
    cs.eval("(set-code \"(define (h x) (+ x 1)) (define stats (compile:per-symbol-dirty-stats "
            "\\\"x\\\"))\")");
    cs.eval("(eval-current)");
