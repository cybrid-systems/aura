...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println("\n--- AC1: (query:jit-interpreter-parity-stats) hash shape ---");
    auto r = cs.eval("(query:jit-interpreter-parity-stats)");
    CHECK(r && aura::compiler::types::is_hash(*r),
          "(query:jit-interpreter-parity-stats) returns a hash");
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto spikes =
        hash_int_field(cs, "(query:jit-interpreter-parity-stats)", "unhandled-opcode-spikes");
    CHECK(spikes == 0,
          std::format("unhandled-opcode-spikes = {} (expected 0 on fresh service)", spikes));
    const auto mm =
        hash_int_field(cs, "(query:jit-interpreter-parity-stats)", "metadata-mismatches");
    CHECK(mm == 0, std::format("metadata-mismatches = {} (expected 0 on fresh service)", mm));
    const auto deopt =
        hash_int_field(cs, "(query:jit-interpreter-parity-stats)", "deopt-on-mutate");
    CHECK(deopt == 0, std::format("deopt-on-mutate = {} (expected 0 on fresh service)", deopt));
    const auto fb =
        hash_int_field(cs, "(query:jit-interpreter-parity-stats)", "fallback-to-interpreter");
    CHECK(fb == 0, std::format("fallback-to-interpreter = {} (expected 0 on fresh service)", fb));
}

static void run_ac3_schema_sentinel(aura::compiler::CompilerService& cs) {
    std::println("\n--- AC3: schema == 720 (drift sentinel) ---");
    const auto schema = hash_int_field(cs, "(query:jit-interpreter-parity-stats)", "schema");
