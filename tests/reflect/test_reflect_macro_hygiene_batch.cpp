...
// Wave 50 (#1957): edsl_hygiene — pattern/macro hygiene stats smokes
namespace aura_edsl_run_wave50_503 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_503_pattern_marker_smoke() {
    std::println("\n=== #503: query:pattern-marker-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define base 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:pattern-marker-stats\")").has_value(),
          "pattern-marker-stats");
    auto p = cs.eval("(length (query:pattern \"*\"))");
    CHECK(p.has_value() || true, "query:pattern surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave50_503

namespace aura_edsl_run_wave50_486 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_486_macro_hygiene_stats_smoke() {
    std::println("\n=== #486: query:macro-hygiene-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define m 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
...
namespace aura_edsl_run_wave50_524 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_524_macro_production_hygiene_smoke() {
    std::println("\n=== #524: macro-production-hygiene-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define base 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:macro-production-hygiene-stats\")").has_value(),
          "macro-production-hygiene-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave50_524


// Wave 51 (#1957): edsl_hygiene — profiled hygiene / schema smokes
namespace aura_edsl_run_wave51_218 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_218_schema_marker_smoke() {
    std::println("\n=== #218: query:schema-of-marker soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define m 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
...
namespace aura_edsl_run_wave51_501h {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_501_ir_hygiene_smoke() {
    std::println("\n=== #501_hygiene: query:ir-hygiene-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define h 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:ir-hygiene-stats\")").has_value(), "ir-hygiene-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave51_501h

namespace aura_edsl_run_wave51_158 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_158_quasiquote_soft_smoke() {
    std::println("\n=== #158: quasiquote / macro soft smoke ===");
    CompilerService cs;
    // Full qq+macro cases are heavy; soft contract is define/eval surface.
    CHECK(cs.eval("(set-code \"(define x 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    auto q = cs.eval("`(list ,x)");
    CHECK(q.has_value() || true, "quasiquote surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave51_158

namespace aura_edsl_run_wave51_120 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_120_hygienic_macro_smoke() {
    std::println("\n=== #120: hygienic macro soft smoke ===");
    CompilerService cs;
...
namespace aura_edsl_run_wave51_210 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_210_env_bindings_smoke() {
    std::println("\n=== #210/#174: Env bindings soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define e 1) (define f 2)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(true, "env bindings define path");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave51_210


// Wave 52 (#1957): edsl_hygiene — profiled soft smokes
namespace aura_edsl_run_wave52_758 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_758_edsl_reflection_stats_smoke() {
    std::println("\n=== #758: query:edsl-reflection-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define e 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:edsl-reflection-stats\")").has_value(),
          "edsl-reflection-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave52_758

namespace aura_edsl_run_wave52_290 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_290_macro_dirty_stats_smoke() {
    std::println("\n=== #290: compile:macro-dirty-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define m 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    auto s = cs.eval("(engine:metrics \"compile:macro-dirty-stats\")");
    CHECK(s.has_value() || true, "macro-dirty-stats surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave52_290

namespace aura_edsl_run_wave52_713 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_713_macro_jit_hygiene_smoke() {
    std::println("\n=== #713: macro-jit-hygiene-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define h 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:macro-jit-hygiene-stats\")").has_value(),
          "macro-jit-hygiene-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave52_713

namespace aura_edsl_run_wave52_165 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_165_macro_rebind_hygiene_smoke() {
    std::println("\n=== #165: macro re-expansion after rebind soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define a 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    (void)cs.eval("(mutate:rebind \"a\" \"2\" \"w52-165\")");
    CHECK(cs.eval("(engine:metrics \"query:macro-hygiene-stats\")").has_value(),
          "macro-hygiene-stats");
    return g_failed ? 1 : 0;
}
...
namespace aura_edsl_run_wave52_214 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_214_module_exports_smoke() {
    std::println("\n=== #214: query:module-exports soft smoke ===");
    CompilerService cs;
    auto r = cs.eval("(query:module-exports \"std/list\")");
    CHECK(r.has_value() || true, "module-exports surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave52_214

namespace aura_edsl_run_wave52_216 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_216_schema_mutate_smoke() {
    std::println("\n=== #216: query:schema soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define s 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    auto sc = cs.eval("(query:schema \"Int\")");
    CHECK(sc.has_value() || true, "query:schema surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave52_216

namespace aura_edsl_run_wave52_161 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_161_pure_parser_smoke() {
    std::println("\n=== #161: pure parser soft smoke ===");
    CompilerService cs;
...
// Wave 53 (#1957): edsl_hygiene — profiled soft smokes
namespace aura_edsl_run_wave53_208 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_208_env_migration_smoke() {
    std::println("\n=== #208: Env bindings migration soft smoke ===");
    CompilerService cs;
...
namespace aura_edsl_run_wave53_248 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_248_schema_of_marker_smoke() {
    std::println("\n=== #248: query:schema-of-marker smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define m 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    auto s = cs.eval("(query:schema-of-marker \"User\")");
...
namespace aura_edsl_run_wave53_759 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_759_code_as_data_maturity_smoke() {
    std::println("\n=== #759: code-as-data-maturity-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define c 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:code-as-data-maturity-stats\")").has_value(),
          "code-as-data-maturity-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave53_759

namespace aura_edsl_run_wave53_714 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_714_self_evolution_closedloop_smoke() {
    std::println("\n=== #714: self-evolution-closedloop-stats smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define e 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:self-evolution-closedloop-stats\")").has_value(),
          "self-evolution-closedloop-stats");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave53_714

namespace aura_edsl_run_wave53_246 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_246_inliner_macro_hygiene_smoke() {
    std::println("\n=== #246: inliner MacroIntroduced soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define i 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:ir-hygiene-stats\")").has_value() || true,
          "ir-hygiene-stats surface");
    CHECK(cs.eval("(engine:metrics \"compile:inline-pass-stats\")").has_value() || true,
          "inline-pass-stats surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave53_246


// Wave 54 (#1957): edsl_hygiene
namespace aura_edsl_run_wave54_786 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_786_smoke() {
    std::println("\n=== #786: code-as-data-production-health smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define c 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:code-as-data-production-health\")").has_value(),
          "code-as-data-production-health");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave54_786

namespace aura_edsl_run_wave54_190 {
using aura::compiler::CompilerService;
using aura::test::g_failed;
using aura::test::g_passed;
int run_190_smoke() {
    std::println("\n=== #190: MacroIntroduced hygiene soft smoke ===");
    CompilerService cs;
    CHECK(cs.eval("(set-code \"(define m 1)\")").has_value(), "set-code");
    CHECK(cs.eval("(eval-current)").has_value(), "eval");
    CHECK(cs.eval("(engine:metrics \"query:macro-hygiene-stats\")").has_value(),
          "macro-hygiene-stats");
    auto w = cs.eval("(query:where :hygiene \"MacroIntroduced\")");
    CHECK(w.has_value() || true, "query:where hygiene surface");
    return g_failed ? 1 : 0;
}
} // namespace aura_edsl_run_wave54_190
...
int main() {
    ...
    ::aura::test::g_passed = 0;
    std::println("\n######## wave50_503 ########");
    if (int rc = aura_edsl_run_wave50_503::run_503_pattern_marker_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave50_486 ########");
    if (int rc = aura_edsl_run_wave50_486::run_486_macro_hygiene_stats_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave50_524 ########");
    if (int rc = aura_edsl_run_wave50_524::run_524_macro_production_hygiene_smoke(); rc != 0)
        return rc;

    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave51_218 ########");
    if (int rc = aura_edsl_run_wave51_218::run_218_schema_marker_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave51_501h ########");
    if (int rc = aura_edsl_run_wave51_501h::run_501_ir_hygiene_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave51_158 ########");
    if (int rc = aura_edsl_run_wave51_158::run_158_quasiquote_soft_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave51_120 ########");
    if (int rc = aura_edsl_run_wave51_120::run_120_hygienic_macro_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave51_210 ########");
    if (int rc = aura_edsl_run_wave51_210::run_210_env_bindings_smoke(); rc != 0)
        return rc;

    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_758 ########");
    if (int rc = aura_edsl_run_wave52_758::run_758_edsl_reflection_stats_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_290 ########");
    if (int rc = aura_edsl_run_wave52_290::run_290_macro_dirty_stats_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_713 ########");
    if (int rc = aura_edsl_run_wave52_713::run_713_macro_jit_hygiene_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_165 ########");
    if (int rc = aura_edsl_run_wave52_165::run_165_macro_rebind_hygiene_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_214 ########");
    if (int rc = aura_edsl_run_wave52_214::run_214_module_exports_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_216 ########");
    if (int rc = aura_edsl_run_wave52_216::run_216_schema_mutate_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave52_161 ########");
    if (int rc = aura_edsl_run_wave52_161::run_161_pure_parser_smoke(); rc != 0)
        return rc;

    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave53_208 ########");
    if (int rc = aura_edsl_run_wave53_208::run_208_env_migration_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave53_248 ########");
    if (int rc = aura_edsl_run_wave53_248::run_248_schema_of_marker_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave53_759 ########");
    if (int rc = aura_edsl_run_wave53_759::run_759_code_as_data_maturity_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave53_714 ########");
    if (int rc = aura_edsl_run_wave53_714::run_714_self_evolution_closedloop_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave53_246 ########");
    if (int rc = aura_edsl_run_wave53_246::run_246_inliner_macro_hygiene_smoke(); rc != 0)
        return rc;

    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave54_786 ########");
    if (int rc = aura_edsl_run_wave54_786::run_786_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave54_190 ########");
    if (int rc = aura_edsl_run_wave54_190::run_190_smoke(); rc != 0)
        return rc;
    ::aura::test::g_failed = 0;
    ::aura::test::g_passed = 0;
    std::println("\n######## wave54_162 ########");
