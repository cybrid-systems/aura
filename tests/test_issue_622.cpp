...
//   - (engine:metrics \"query:atomic-batch-rollback-stats\") from #529
...
int aura_issue_622_run() {
    ...
    // AC1: (engine:metrics \"query:atomic-batch-stats-hash\") returns a hash with the
    // documented 4 fields.
    {
        std::println("\n--- AC1: (engine:metrics \"query:atomic-batch-stats-hash\") shape ---");
    ...
    // the existing observability surface).
    {
        ...
        CHECK(
            s529.has_value(),
            "(engine:metrics \"query:atomic-batch-rollback-stats\") reachable (#529 back-compat)");
    }

    // AC3: concurrent reads of (engine:metrics \"query:atomic-batch-stats-hash\")
