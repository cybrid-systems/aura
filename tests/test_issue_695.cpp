...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (query:eda-sv-closedloop-stress-stats) '{}')", key));
    ...
    while (flat.size() < 5000) {
        for (int clk = 0; clk < 10 && flat.size() < 5000; ++clk) {
            ...
            for (int mp = 0; mp < 4; ++mp) {
                const std::vector<aura::ast::SymId> ports{
                    pool.intern("sig_a"), pool.intern("sig_b")};
                body.push_back(flat.add_modport(
                    pool.intern(std::format("mp_{}_{}", batch, mp)), ports));
            }
            (void)flat.add_interface(
                pool.intern(std::format("iface_{}_{}", batch, clk)), body);
            const auto prop = flat.add_property(
                pool.intern(std::format("p_{}_{}", batch, clk)),
                pool.intern("req ##1 ack"));
            const std::vector<aura::ast::SymId> bins{pool.intern("b0"),
                                                     pool.intern("b1")};
            const auto cp = flat.add_coverpoint(
                pool.intern(std::format("v_{}_{}", batch, clk)), bins);
...
int main() {
    ...
    // AC1: stats hash fields
    {
        ...
        CHECK(stat_int(cs, "feedback-mutate-success") >= 0,
              "feedback-mutate-success present");
    ...
    // AC2: synthetic SoC hierarchy scale
    {
        ...
        if (ws) {
            CHECK(ws->size() >= 5000,
                  std::format("workspace has 5k+ nodes (got {})", ws->size()));
            std::uint64_t sva_nodes = 0;
            for (aura::ast::NodeId id = 0; id < ws->size(); ++id) {
                const auto tag = ws->get(id).tag;
                if (tag == aura::ast::NodeTag::Property ||
                    tag == aura::ast::NodeTag::Coverpoint ||
                    tag == aura::ast::NodeTag::Interface)
                    ++sva_nodes;
            }
            CHECK(sva_nodes >= 100,
                  std::format("SVA/interface nodes present (got {})", sva_nodes));
        }
    }
    ...
    // AC3: eda:demo-sv-self-evolution
    {
        ...
        CHECK(r && aura::compiler::types::is_int(*r) &&
              ...
              std::format("demo returned {} successes",
                          r ? aura::compiler::types::as_int(*r) : -1));
        const auto cycles_after = stat_int(cs, "evolution-cycles");
        CHECK(cycles_after > cycles_before,
              std::format("evolution-cycles grew ({} -> {})",
                          cycles_before, cycles_after));
        const auto rate = stat_int(cs, "verification-convergence-rate");
        CHECK(rate >= 50,
              std::format("verification-convergence-rate >= 50% (got {}%)", rate));
        CHECK(stat_int(cs, "corruption-detected") == 0,
              "no corruption detected during demo");
    }
    ...
    // AC4: stats:count
    {
        ...
        CHECK(count && aura::compiler::types::is_int(*count) &&
                  aura::compiler::types::as_int(*count) == 76,
              "stats:count == 76");
    }
    ...
    // AC5: fiber stress + chaos (GC safepoint)
    {
        ...
        CHECK(ok_count.load() > 0,
              std::format("fiber stress produced {} successful feedback loops",
                          ok_count.load()));
        CHECK(stat_int(cs, "corruption-detected") == 0,
              "no corruption after fiber stress");
    }
