...
def main():
    ...
    for s in range(n_sessions):
        ...
        print(
            f"{st['ok']}/{st['ok'] + st['error'] + st['crash']} ({pct:.0f}%) [errors={st['error']} crash={st['crash']}]"
        )

    print(f"\n{'=' * 60}")
    print(f"  Summary: {total['ok']} ok, {total['error']} error, {total['crash']} crash, {total['timeout']} timeout")
