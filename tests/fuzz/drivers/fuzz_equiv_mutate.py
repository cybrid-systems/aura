...
from common import CORPUS as _CORPUS
...
def run_aura(code, backend="tree-walk"):
    ...
    try:
        r = subprocess.run([AURA] + flags, input=code, capture_output=True, text=True, timeout=TIMEOUT)
...
def gen_simple_program():
    ...
    pattern = rng.choice(
        [
            ...
            # Nested arithmetic
            lambda: (
                f"(display (+ (* {rng.randint(1, 10)} {rng.randint(1, 10)}) (- {rng.randint(10, 50)} {rng.randint(1, 10)})))"
            ),
            lambda: (
                f"(display (* (+ {rng.randint(1, 10)} {rng.randint(1, 10)}) (- {rng.randint(10, 50)} {rng.randint(1, 10)})))"
            ),
            # Conditionals
            lambda: (
                f"(display (if (> {rng.randint(1, 10)} {rng.randint(1, 10)}) {rng.randint(1, 100)} {rng.randint(1, 100)}))"
            ),
            ...
            # Stdlib (with require)
            lambda: "(require std/list all:)(display (length '(1 2 3 4 5)))",
            lambda: "(require std/list all:)(display (map (lambda (x) (+ x 1)) '(1 2 3)))",
            lambda: "(require std/list all:)(display (filter (lambda (x) (> x 2)) '(1 2 3 4 5)))",
            lambda: "(require std/list all:)(display (reverse '(1 2 3)))",
            # Pair operations
            lambda: "(display (car (cons 1 2)))",
            lambda: "(display (cdr (cons 1 2)))",
            # Composition
            lambda: (
                f"(display ((lambda (f g x) (f (g x))) (lambda (y) (* y 2)) (lambda (z) (+ z 1)) {rng.randint(1, 10)}))"
            ),
            # Factorial (small)
            lambda: "(define (fact n) (if (= n 0) 1 (* n (fact (- n 1)))))(display (fact 5))",
            # Fibonacci (small)
            lambda: "(define (fib n) (if (< n 2) n (+ (fib (- n 1)) (fib (- n 2)))))(display (fib 8))",
            # Map with named function
            lambda: "(require std/list all:)(define (double x) (* x 2))(display (map double '(1 2 3 4)))",
            # Nested defines
            lambda: "(define (f x) (+ x 1))(define (g y) (* y 2))(display (g (f 5)))",
    ...
    for i, tok in enumerate(tokens):
        if tok == "(":
            ...
            if depth == 0:
                ...
                if len(expr) >= 2 and expr[1] not in (
                    "begin",
                    "define",
                    "define-type",
                    "defmacro",
    ...
    for i, tok in enumerate(tokens):
        if tok == "(" and i + 1 < len(tokens) and tokens[i + 1] == "let":
            ...
            if body_str.startswith("(let"):
                ...
                for _idx, p in enumerate(all_pairs):
...
def eta_expand(code, tokens):
    ...
    RESERVED = {
        ...
        # #2628: bare query:* aliases purged — keep dispatcher + remaining public names
        "query",
        "query:node-type",
        "query:calls",
        "query:siblings",
...
def constant_reassoc(code, tokens):
    ...
    for i, tok in enumerate(tokens):
        if tok == "(" and i + 1 < len(tokens) and tokens[i + 1] in ("+", "*"):
            ...
            if len(operands) >= 3 and len(operands[0]) >= 3 and operands[0][0] == "(" and operands[0][1] == op:
...
def main():
            ...
            if f.endswith(".sexpr"):
                with open(os.path.join(CORPUS_DIR, f)) as fp:
                    ...
                    if (
                        ...
                        and "`" not in code
                        and ",@" not in code
                        and "define" not in code
                        and "defmacro" not in code
    ...
    for i in range(n_programs):
        if (i + 1) % 50 == 0:
            ...
            print(
                f"  [{i + 1:4d}/{n_programs}]  {results['pass']} pass, "
        ...
        code = rng.choice(corpus_seeds) if corpus_seeds and i % 3 == 0 else gen_program_for_mutate()
        result = test_one_equivalence(code)
        if result == "fail":
            print(f"  ! [{i + 1}] FAIL — see summary")

    elapsed = time.time() - start
    (results["pass"] + results["fail_equiv"] + results["fail_crash"] + results["fail_timeout"])

    print(f"\n{'=' * 60}")
    print("  Equivalence Mutation Fuzz Summary")
    print(f"{'=' * 60}")
