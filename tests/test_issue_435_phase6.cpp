...
bool test_cpp_emit_class_with_base() {
    ...
    s = emit_class(make_class("packet",
        "base_pkt",
        std::vector<std::string>{"logic [7:0] data;"}));
    CHECK(s.find("class packet") != std::string::npos,
          "starts with 'class packet'");
    CHECK(s.find("extends base_pkt") != std::string::npos,
          "contains 'extends base_pkt'");
    CHECK(s.find("logic [7:0] data;") != std::string::npos,
          "contains item body");
    CHECK(s.find("endclass") != std::string::npos,
          "ends with 'endclass'");
...
bool test_cpp_emit_class_no_base() {
    ...
    s = emit_class(make_class("simple", "",
        std::vector<std::string>{"int x;", "int y;"}));
    CHECK(s.find("class simple") != std::string::npos,
          "starts with 'class simple'");
    CHECK(s.find("extends") == std::string::npos,
          "no 'extends' when base is empty");
...
bool test_cpp_emit_class_empty() {
    ...
    CHECK(s.find("class empty") != std::string::npos,
          "starts with 'class empty'");
    CHECK(s.find("endclass") != std::string::npos,
          "ends with 'endclass'");
...
bool test_list_ir_works() {
    ...
    CHECK(s.find("class packet") != std::string::npos,
          "list-based contains 'class packet'");
    CHECK(s.find("extends base_pkt") != std::string::npos,
          "list-based contains 'extends base_pkt'");
    CHECK(s.find("logic [7:0] data;") != std::string::npos,
          "list-based contains item body");
    CHECK(s.find("endclass") != std::string::npos,
          "list-based ends with 'endclass'");
...
int aura_issue_435_phase6_run() { return aura_issue_435_phase6_detail::run_tests(); }

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() { return aura_issue_435_phase6_run(); }
#endif