...
namespace eda {
    ...
    using Value =
        std::variant<std::monostate, int64_t, std::string, bool, std::vector<class IrNode>>;
    ...
    public:
        ...
        IrNode(Tag t, std::vector<Value> f)
            : tag(t)
            , fields(std::move(f)) {}
        ...
        template <typename T> T const& get(std::size_t i) const { return std::get<T>(fields[i]); }
    };
    ...
    IrNode make_module(std::string name, std::vector<IrNode> ports, std::vector<IrNode> body) {
        return IrNode(Tag::Module, {std::move(name), std::move(ports), std::move(body)});
    }
    bool is_module(IrNode const& n) {
        return n.is(Tag::Module);
    }
    std::string const& module_name(IrNode const& n) {
        return n.get<std::string>(0);
    }
    ...
    bool is_port(IrNode const& n) {
        return n.is(Tag::Port);
    }
    std::string const& port_name(IrNode const& n) {
        return n.get<std::string>(0);
    }
    std::string const& port_direction(IrNode const& n) {
        return n.get<std::string>(1);
    }
    int64_t port_width(IrNode const& n) {
        return n.get<int64_t>(2);
    }
    ...
    bool is_wire(IrNode const& n) {
        return n.is(Tag::Wire);
    }
    std::string const& wire_name(IrNode const& n) {
        return n.get<std::string>(0);
    }
    int64_t wire_width(IrNode const& n) {
        return n.get<int64_t>(1);
    }
    ...
    bool is_assign(IrNode const& n) {
        return n.is(Tag::Assign);
    }
    ...
    IrNode expr_int(int64_t v) {
        ...
        literal.fields.push_back(std::vector<IrNode>{});
        std::get<std::vector<IrNode>>(literal.fields[1]).push_back(IrNode(Tag::Expr, {v}));
        return literal;
    }
    bool is_expr(IrNode const& n) {
        return n.is(Tag::Expr);
    }
    std::string const& expr_op(IrNode const& n) {
        return n.get<std::string>(0);
    }
    ...
    bool is_always(IrNode const& n) {
        return n.is(Tag::Always);
    }
    ...
    bool is_signal(IrNode const& n) {
        return n.is(Tag::Signal);
    }
    ...
    bool is_sensitivity(IrNode const& n) {
        return n.is(Tag::Sensitivity);
    }
    ...
    std::string display_expr(IrNode const& e) {
        ...
        if (op == "symbol")
            return expr_symbol_name(e);
        if (op == "int")
            return std::to_string(expr_int_value(e));
    ...
    std::string display_port(IrNode const& p) {
        ...
        if (port_width(p) > 1)
            s += " [" + std::to_string(port_width(p)) + "-1:0]";
        return s;
    }
    ...
    std::string display_wire(IrNode const& w) {
        std::string s = "wire ";
        if (wire_width(w) > 1)
            s += "[" + std::to_string(wire_width(w)) + "-1:0] ";
        s += wire_name(w);
        s += ";";
        return s;
    }
    ...
    std::string display_assign(IrNode const& a) {
        return "assign " + display_expr(assign_lhs(a)) + " = " + display_expr(assign_rhs(a)) + ";";
    }

    std::string emit_expr(IrNode const& e);

    std::string emit_infix(std::string_view op_str, std::vector<IrNode> const& args) {
        if (args.empty())
            return "";
        if (args.size() == 1)
    ...
    std::string emit_assign(IrNode const& a) {
        return "assign " + emit_expr(assign_lhs(a)) + " = " + emit_expr(assign_rhs(a)) + ";";
    }

    std::string emit_sensitivity(IrNode const& s) {
        return std::get<std::string>(s.fields[0]) + " " + std::get<std::string>(s.fields[1]);
    }

    std::string emit_always_body_stmt(IrNode const& item) {
        if (is_assign(item))
            return emit_assign(item);
        if (is_wire(item))
            return emit_wire(item);
        return "";
    }
    ...
    std::string emit_always_body(std::vector<IrNode> const& body) {
        if (body.empty())
            return "end\n";
        if (body.size() == 1)
            return emit_always_body_stmt(body[0]) + "end\n";
        return emit_always_body_stmt(body[0]) + "\n" +
               emit_always_body({body.begin() + 1, body.end()});
    }
    ...
    std::string emit_module_body(std::vector<IrNode> const& body) {
        if (body.empty())
            return "";
        if (body.size() == 1)
            return emit_body_item(body[0]);
        return emit_body_item(body[0]) + emit_module_body({body.begin() + 1, body.end()});
    }
    ...
    IrNode assign_rename_sym(IrNode a, std::string_view old_name, std::string_view new_name) {
        return make_assign(expr_rename_sym(assign_lhs(a), old_name, new_name),
                           expr_rename_sym(assign_rhs(a), old_name, new_name));
    }

    IrNode body_item_rename_sym(IrNode item, std::string_view old_name, std::string_view new_name) {
        if (is_wire(item))
            return wire_rename_sym(item, old_name, new_name);
        if (is_assign(item))
            return assign_rename_sym(item, old_name, new_name);
        return item;
    }

    IrNode mutate_rename_symbol(std::string_view old_name, std::string_view new_name, IrNode m) {
    ...
    IrNode mutate_insert_clock_gating(IrNode m, std::string_view clk, std::string_view en) {
        std::vector<IrNode> body = module_body(m);
        body.insert(body.begin(), make_wire("clk_gated", 1));
        body.insert(body.begin() + 1, make_assign(expr_symbol("clk_gated"),
    ...
    bool expr_equal(IrNode const& e1, IrNode const& e2) {
        if (expr_op(e1) != expr_op(e2))
            return false;
        if (expr_op(e1) == "symbol")
            return expr_symbol_name(e1) == expr_symbol_name(e2);
        if (expr_op(e1) == "int")
            return expr_int_value(e1) == expr_int_value(e2);
        auto const& a1 = expr_args(e1);
        auto const& a2 = expr_args(e2);
        if (a1.size() != a2.size())
            return false;
        for (std::size_t i = 0; i < a1.size(); ++i) {
            if (!expr_equal(a1[i], a2[i]))
                return false;
        }
        return true;
    }
    ...
    IrNode port_to_typed_signal(IrNode const& p) {
        return IrNode(Tag::Expr, {std::string("typed-signal"), port_name(p), port_width(p)});
    }

    IrNode wire_to_typed_signal(IrNode const& w) {
        return IrNode(Tag::Expr, {std::string("typed-signal"), wire_name(w), wire_width(w)});
    }

    std::vector<IrNode> query_typed_signals(IrNode const& m) {
        std::vector<IrNode> result;
        for (auto const& p : module_ports(m))
            result.push_back(port_to_typed_signal(p));
        for (auto const& w : query_wires(m))
            result.push_back(wire_to_typed_signal(w));
        return result;
    }
    ...
    bool assign_widths_ok(IrNode const& a, IrNode const& m) {
        ...
        if (expr_op(lhs) != "symbol" || expr_op(rhs) != "symbol")
            return true;
    ...
    bool check_module_types(IrNode const& m) {
        for (auto const& a : query_assigns(m)) {
            if (!assign_widths_ok(a, m))
                return false;
        }
        return true;
    }

    std::optional<IrNode> mutate_insert_clock_gating_checked(IrNode m, std::string_view clk,
                                                             std::string_view en) {
...
bool test_display() {
    ...
    CHECK(s.find("module counter") != std::string::npos, "display contains 'module counter'");
    CHECK(s.find("input clk") != std::string::npos, "display contains 'input clk'");
    CHECK(s.find("output q [8-1:0]") != std::string::npos, "display contains 'output q [8-1:0]'");
    ...
    CHECK(s.find("endmodule") != std::string::npos, "display contains 'endmodule'");
...
bool test_query_helpers() {
    ...
    if (!assigns.empty()) {
        CHECK(expr_op(assign_lhs(assigns[0])) == "symbol", "queried assign lhs is a symbol");
    }
...
bool test_predicate_disjoint() {
    ...
    CHECK(is_module(m) && !is_port(m) && !is_wire(m) && !is_assign(m) && !is_expr(m),
          "module matches only is_module");
    CHECK(is_port(p) && !is_module(p) && !is_wire(p) && !is_assign(p) && !is_expr(p),
          "port matches only is_port");
    CHECK(is_wire(w) && !is_module(w) && !is_port(w) && !is_assign(w) && !is_expr(w),
          "wire matches only is_wire");
    CHECK(is_assign(a) && !is_module(a) && !is_port(a) && !is_wire(a) && !is_expr(a),
          "assign matches only is_assign");
    CHECK(is_expr(e) && !is_module(e) && !is_port(e) && !is_wire(e) && !is_assign(e),
          "expr matches only is_expr");
...
bool test_nested_expr() {
    ...
    // Slice: q[7:0]
    auto slice = make_expr("slice", {expr_symbol("q"), expr_int(7), expr_int(0)});
...
bool test_cycle2_mutate_extract_common() {
    ...
    if (!query_wires(extracted).empty())
        CHECK(wire_name(query_wires(extracted)[0]) == "common_expr", "common wire name");
    return true;
}
...
bool test_cycle3_emit_verilog() {
    ...
    CHECK(s.find("module counter") != std::string::npos, "emit contains module header");
    CHECK(s.find("wire [8-1:0] q_next;") != std::string::npos, "emit contains wire decl");
    CHECK(s.find("assign q = q_next;") != std::string::npos, "emit contains assign");
    CHECK(s.find("endmodule") != std::string::npos, "emit contains endmodule");
    CHECK(s.find("eda:module") == std::string::npos, "emit does not leak IR tags");
    return true;
}
...
bool test_cycle3_emit_always() {
    ...
    auto always_blk = make_always({make_sensitivity("posedge", "clk")}, body);
    ...
    CHECK(s.find("always @(posedge clk)") != std::string::npos, "emit contains sensitivity list");
    CHECK(s.find("assign q = (q + 1);") != std::string::npos, "emit contains infix assignment");
    CHECK(s.find("end\nendmodule") != std::string::npos, "emit closes always block and module");
    return true;
}
...
bool test_cycle4_check_module_types() {
    ...
    auto ok =
        make_module("ok", {make_port("q", "output", 8)},
                    {make_wire("q_next", 8), make_assign(expr_symbol("q"), expr_symbol("q_next"))});
    CHECK(check_module_types(ok), "matching symbol widths pass");

    auto bad = make_module("bad", {make_port("q", "output", 8)},
...
bool test_cycle4_pipeline() {
    ...
    auto m =
        make_module("counter", {make_port("clk", "input", 1), make_port("en", "input", 1)}, {});
...
int aura_issue_182_run() {
    return aura_issue_182_detail::run_tests();
}
