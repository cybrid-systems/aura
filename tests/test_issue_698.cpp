...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (engine:metrics \"query:hardware-backend-commercial-stats\") '{}')", key));
...
int aura_issue_698_run() {
    ...
    // AC1: query:hardware-backend-commercial-stats hash fields
    {
        ...
        auto stats = cs.eval("(engine:metrics \"query:hardware-backend-commercial-stats\")");
