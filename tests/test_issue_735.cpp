...
//   - #717 (query:fiber-boundary-violation-stats) — fiber/Guard
...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println("\n--- AC1: (query:macro-provenance-stats) hash shape ---");
    auto r = cs.eval("(query:macro-provenance-stats)");
    CHECK(r && aura::compiler::types::is_hash(*r), "(query:macro-provenance-stats) returns a hash");
    ...
    for (const auto& k : keys) {
        auto f = cs.eval(std::format("(hash-ref (query:macro-provenance-stats) '{}')", k));
...
static void run_ac3_schema_sentinel(aura::compiler::CompilerService& cs) {
    std::println("\n--- AC3: schema == 735 (drift sentinel) ---");
    const auto schema = hash_int_field(cs, "(query:macro-provenance-stats)", "schema");
...
static void run_ac4_bump_accessible(aura::compiler::CompilerService& cs) {
    ...
    CHECK(consults == 3,
          std::format("after 3 is-macro-introduced-consult bumps: is-macro-introduced-consults "
                      ...
                      consults));
    CHECK(captured == 4,
          std::format("after 4 provenance-captured bumps: provenance-captured = {} "
                      "(expected 4)",
                      captured));
    CHECK(dirty == 2,
          std::format("after 2 dirty-impact bumps: dirty-impact-on-macro-subtree = {} "
                      "(expected 2)",
                      dirty));
    CHECK(rollback == 1,
          std::format("after 1 rollback-success bump: rollback-success = {} (expected 1)",
                      rollback));
}
