...
def run_single_task_intend(model, base_url, api_key, name, prompt, expected, stdlib, api_ref):
    ...
    lines.append('                 (n-calls (length (query:find "Call")))')
    lines.append('                 (n-vars (length (query:find "Variable"))))')
    lines.append('            (string-append')
    lines.append('              "=== Source (AST formatted) ===\\n" src')
    lines.append('              "\\n=== AST Stats ===\\ncall-sites: "')
    lines.append('              (number->string n-calls) " variables: "')
    lines.append('              (number->string n-vars)')
    lines.append('              "\\n=== Error ===\\n" err')
    lines.append('              "\\n=== Goal ===\\n" goal)))')
    lines.append('    "temperature" 0.3')
    lines.append('    "max_tokens" 4096)))) "content")))')
