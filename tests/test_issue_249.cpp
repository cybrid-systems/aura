...
bool test_captured_ref_is_valid() {
    ...
    auto r = try_run(cs, "(let ((stable (car (query:children-stable 6))))"
...
bool test_ref_stale_after_mutation() {
    ...
    CHECK(valid == 0 || valid == 1, "mutate:check-stable-ref returns 0 or 1 after a mutation");
    return true;
}
...
bool test_bad_args() {
    ...
    CHECK_EQ_INT(aura::compiler::types::as_int(r2.v), 1, "string-arg returns a pair (error)");
    return true;
}
...
bool test_out_of_range() {
    ...
    CHECK_EQ_INT(aura::compiler::types::as_int(r.v), 1, "out-of-range returns a pair (error)");
    return true;
}
...
int aura_issue_249_run() {
    return aura_issue_249_detail::run_tests();
}
