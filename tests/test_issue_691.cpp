...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (query:coercion-narrowing-stats) '{}')", key));
...
int main() {
    ...
    // AC2: Dynamic + predicate narrowing + mutate:rebind
    {
        ...
        (void)cs.eval("(typecheck-current)");
        auto f5 = cs.eval("(f 5)");
        CHECK(f5 && aura::compiler::types::is_int(*f5) &&
                  aura::compiler::types::as_int(*f5) == 17,
        ...
        CHECK(opp_after > opp_before,
              std::format("post-narrow-elim-opportunities grew ({} -> {})",
                          opp_before, opp_after));
        CHECK(blame_after >= blame_before,
              std::format("blame-chain-hits non-decreasing ({} -> {})",
                          blame_before, blame_after));
        CHECK(elim_after >= elim_before,
              std::format("cast-elim-from-narrow non-decreasing ({} -> {})",
                          elim_before, elim_after));
    }
    ...
    // AC3: query:coercion-stats regression
    {
        ...
        CHECK(aura::compiler::types::as_int(*elim) >= 0,
              "coercion-elim-stats non-negative");
    }
    ...
    // AC6: fiber stress
    {
        ...
        CHECK(ok_count.load() > 0,
              std::format("fiber stress produced {} successful f 3 evals",
                          ok_count.load()));
    }
