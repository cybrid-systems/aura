...
// matching.
static std::string read_file_head(const std::string& path, std::size_t n_lines = 200) {
...
bool test_sanitizer_matrix_script_exists() {
    ...
    CHECK(!contents.empty(), "tests/run_sanitizer_matrix.sh exists");
    if (contents.empty())
        return false;
    // All 3 sanitizers supported in the case statement.
    CHECK(contents.find("thread|address|undefined|all|both|coverage") != std::string::npos,
          "case statement lists thread/address/undefined/all/both/coverage");
    // Per-sanitizer build directories.
    CHECK(contents.find("build_tsan") != std::string::npos, "TSan build dir documented");
    CHECK(contents.find("build_asan") != std::string::npos, "ASan build dir documented");
    CHECK(contents.find("build_ubsan") != std::string::npos, "UBSan build dir documented");
    CHECK(contents.find("build_cov") != std::string::npos, "coverage build dir documented");
    // Per-sanitizer compiler flag sets.
    CHECK(contents.find("TSAN_FLAGS") != std::string::npos, "TSan compiler flags defined");
    CHECK(contents.find("ASAN_FLAGS") != std::string::npos, "ASan compiler flags defined");
    CHECK(contents.find("UBSAN_FLAGS") != std::string::npos, "UBSan compiler flags defined");
    // Syntax check.
    CHECK(shell_syntax_ok(script), "script is syntactically valid (bash -n)");
    return true;
}
...
bool test_coverage_toolchain() {
    ...
    CHECK(contents.find("-lgcov") != std::string::npos, "linker flag -lgcov present");
    ...
    CHECK(contents.find("fiber.cpp") != std::string::npos, "fiber.cpp covered");
    CHECK(contents.find("aura_jit_bridge.cpp") != std::string::npos, "aura_jit_bridge.cpp covered");
    return true;
}
...
int aura_issue_325_run() {
    return aura_issue_325_detail::run_tests();
}

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() {
    return aura_issue_325_run();
}
#endif