...
bool test_cpp_constraint_ir_direct() {
    ...
    ConstraintIR c = make_constraint("valid_len", std::vector<std::string>{"len < 128", "len > 0"});
...
bool test_cpp_emit_constraint() {
    ...
    s = emit_constraint(
        make_constraint("valid_len", std::vector<std::string>{"len < 128", "len > 0"}));
    ...
    CHECK(s.find("constraint single") != std::string::npos, "single-expr: starts with name");
    ...
    CHECK(s.find("constraint empty") != std::string::npos, "empty: starts with name");
    CHECK(s.find("}") != std::string::npos, "empty: ends with '}'");
    // Empty body should NOT have a separator '; '
    CHECK(s.find(";") == std::string::npos, "empty: no separator inside braces");
...
bool test_debug_constraint() {
    ...
    std::string s = debug_constraint(make_constraint("c", std::vector<std::string>{"a", "b"}));
    CHECK(s.find("constraint(name=c") != std::string::npos, "debug prefix");
    CHECK(s.find("exprs=[a,b]") != std::string::npos, "debug body");
...
bool test_list_ir_works() {
    ...
    auto s = run_string(cs, "(eda:emit-constraint "
                            "  (make-eda:constraint 'valid_len "
                            "    (list \"len < 128\" \"len > 0\")))");
    CHECK(!s.empty(), "list-based emit returns non-empty");
    CHECK(s.find("constraint valid_len") != std::string::npos, "list-based contains name");
    CHECK(s.find("len < 128") != std::string::npos, "list-based contains expr 1");
    CHECK(s.find("len > 0") != std::string::npos, "list-based contains expr 2");
    CHECK(s.find("}") != std::string::npos, "list-based ends with '}'");
...
bool test_list_ir_predicates() {
    ...
    CHECK(r.has_value(), "predicate eda:constraint? returns non-empty (symbol #t / #f)");

    auto s = run_string(cs, "(eda:name-str (eda:constraint-name c1))");
    CHECK(s == "len_limit", "eda:constraint-name returns the name as symbol-string");
...
int aura_issue_435_phase5_run() {
    return aura_issue_435_phase5_detail::run_tests();
}

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() {
    return aura_issue_435_phase5_run();
}
#endif