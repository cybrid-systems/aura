...
bool check_predecessor_coverage_ac2_ac3() {
    std::println("\n--- AC2+AC3: predecessor coverage (skip_macro, only_macro, IR/JIT, atomic batch marker) ---");
    ...
    bool has_inverse = contains(qm, "only_macro_introduced_") &&
                       contains(qm, "recursive_user_skipped_") &&
                       contains(qm, "Issue #1650");
    ...
    bool has_ir_marker = contains(pq, "query:ir-marker-stats") ||
                         contains(pq, "\"ir_marker_stats_query_total\"");
...
bool check_no_new_primitive() {
    std::println("\n--- Composition guard: no new primitive registered (\\u539f\\u8bed\\u6700\\u5c0f\\u5316 directive) ---");
    std::string pq = read_file("src/compiler/evaluator_primitives_query.cpp");
    ...
    if (has_new_hygiene_stats_prim || has_new_template_marker_stats_prim) {
        std::println("FAIL: new primitive registered (violates \\u539f\\u8bed\\u6700\\u5c0f\\u5316 directive) "
                     "(hygiene_stats={} template_marker_stats={})",
                     has_new_hygiene_stats_prim, has_new_template_marker_stats_prim);
        return false;
    }
    std::println("OK: no new primitive registered; composition into existing primitives per directive");
    return true;
}
...
int main() {
    ...
    int rc = 0;
    if (!check_primitives_style_doc_ac1())  rc = 1;
    if (!check_predecessor_coverage_ac2_ac3()) rc = 1;
    if (!check_design_doc_present())       rc = 1;
    if (!check_no_new_primitive())          rc = 1;

    if (rc == 0) {
        std::println("\n#1653 scope-limited-progressive-ship Phase 1 \u2014 all AC checks green \u2705\n"
                     "    AC3 full #1047 hygiene completion + optional query_engine.ixx extraction \u2192 #1689\n"
