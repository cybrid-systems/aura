...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (query:occurrence-typing-mutate-stats) '{}')", key));
...
int main() {
    ...
    // AC1: stats hash fields
    {
        ...
        CHECK(stat_int(cs, "deep-narrow-refreshes") >= 0,
              "deep-narrow-refreshes present");
        CHECK(stat_int(cs, "blame-refreshes") >= 0, "blame-refreshes present");
        CHECK(stat_int(cs, "stale-narrowing-caught") >= 0,
              "stale-narrowing-caught present");
    ...
    // AC2: deep and/or/not + typecheck + mutate:rebind
    {
        ...
        CHECK(deep_after > deep_before,
              std::format("deep-narrow-refreshes grew ({} -> {})",
                          deep_before, deep_after));
        CHECK(blame_after >= blame_before,
              std::format("blame-refreshes non-decreasing ({} -> {})",
