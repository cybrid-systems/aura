...
//   - #458 (query:pattern-hygiene-stats) — different scope (basic count)
...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    ...
    for (const auto& k : keys) {
        auto f = cs.eval(
            std::format("(hash-ref (query:macro-hygiene-provenance-stats) '{}')", k));
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto captured = hash_int_field(cs, "(query:macro-hygiene-provenance-stats)",
                                         "provenance-captured");
    CHECK(captured == 0,
          std::format("provenance-captured = {} (expected 0 on fresh service)", captured));
    const auto inliner = hash_int_field(cs, "(query:macro-hygiene-provenance-stats)",
                                        "inliner-policy-violations");
    CHECK(inliner == 0,
          std::format("inliner-policy-violations = {} (expected 0 on fresh service)", inliner));
    const auto prov_v = hash_int_field(cs, "(query:macro-hygiene-provenance-stats)",
                                       "provenance-violations");
    CHECK(prov_v == 0,
          std::format("provenance-violations = {} (expected 0 on fresh service)", prov_v));
    const auto dirty = hash_int_field(cs, "(query:macro-hygiene-provenance-stats)",
                                      "hygiene-dirty-impact");
    CHECK(dirty == 0,
...
static void run_ac4_bump_accessible(aura::compiler::CompilerService& cs) {
    ...
    CHECK(inliner == 3,
          std::format(
              "after 3 inliner-policy-violation bumps: inliner-policy-violations = {} "
              "(expected 3)",
              inliner));
    // Cross-reference fields must remain 0 since we only bumped the 2 new atomics.
    CHECK(prov_v == 0,
          std::format(
              "provenance-violations = {} (expected 0 after only new bumps)", prov_v));
    CHECK(dirty == 0,
