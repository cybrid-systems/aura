...
//     total)
//   - (query:scheduler-steal-bias-stats) (#645) — per-steal
...
static std::int64_t hash_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(
        std::format("(hash-ref (query:scheduler-stealbudget-yield-class-stats) '{}')", key));
...
int main() {
    ...
    {
        std::println("\n--- AC1: (query:scheduler-stealbudget-yield-class-stats) shape ---");
        auto h = cs.eval("(query:scheduler-stealbudget-yield-class-stats)");
    ...
    // distinct from #645's `-steal-bias-` (per-steal LIFO/FIFO).
    {
        ...
        auto new_p = cs.eval("(query:scheduler-stealbudget-yield-class-stats)");
        auto old_706 = cs.eval("(query:scheduler-stealbudget-adaptive-stats)");
        auto old_645 = cs.eval("(query:scheduler-steal-bias-stats)");
        CHECK(new_p.has_value(), "new primitive (query:scheduler-stealbudget-yield-class-stats) "
                                 "reachable (-yield-class- midfix)");
        CHECK(old_706.has_value(), "existing #706 (query:scheduler-stealbudget-adaptive-stats) "
                                   "still reachable (5-field summary)");
        CHECK(old_645.has_value(), "existing #645 (query:scheduler-steal-bias-stats) still "
        ...
        const auto check_new_field = [&](std::string_view k) {
            auto r = cs.eval(
                std::format("(hash-ref (query:scheduler-stealbudget-yield-class-stats) '{}')", k));
    ...
    // scaffolding atomics.
    {
        ...
        auto worker = [&] {
            for (int i = 0; i < k_iters; ++i) {
                ...
                auto r = cs.eval("(query:scheduler-stealbudget-yield-class-stats)");
                if (r.has_value())
