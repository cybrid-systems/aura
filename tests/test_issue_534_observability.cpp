...
static std::int64_t hash_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (engine:metrics \"query:arena-production-compaction-stats\") '{}')", key));
...
int aura_issue_534_observability_run() {
    ...
    // AC1: query:arena-production-compaction-stats returns hash
    {
        ...
        auto stats = cs.eval("(engine:metrics \"query:arena-production-compaction-stats\")");
...
int main() {
    return aura_issue_534_observability_run();
}
#endif
