...
bool test_aura_primitive_returns_hash() {
    ...
    for (const char* key : {"bump-generation-count", "stable-ref-invalidations",
        ...
        if (!rv || !aura::compiler::types::is_int(*rv) ||
            ...
            std::println("  FAIL: hash-ref h {} did not return 0 (val={})",
                         key, rv ? rv->val : -1);
            ++g_failed;
        } else {
...
bool test_is_valid_check_count() {
    ...
    for (int i = 0; i < 5; ++i) {
        auto r = cs.eval("x");
        if (!r) { std::println("  FAIL: eval x #{} failed", i); ++g_failed; return false; }
    }
...
bool test_atomic_batch_commits_via_primitive() {
    ...
    auto r3 = cs.eval("(begin (mutate:atomic-batch (lambda () (mutate:rebind \"x\" \"6\"))) (eval-current))");
    ...
    if (!rg2 || !aura::compiler::types::is_int(*rg2)) {
        std::println("  FAIL: hash-ref after batch failed"); ++g_failed; return false;
    }
    auto after = static_cast<std::uint64_t>(aura::compiler::types::as_int(*rg2));
    ...
    CHECK(after >= baseline,
          "atomic-batch-commits non-negative (counter wired up; batch may or may not have committed)");
    return true;
}
...
int aura_issue_255_run() { return aura_issue_255_detail::run_tests(); }
