...
//   - (engine:metrics \"query:stable-ref-provenance-sv-stats-hash\") (#631) —
...
int aura_issue_641_run() {
    ...
    {
        std::println(
            "\n--- AC1: (engine:metrics \"query:stable-ref-provenance-sv-stats\") shape ---");
        auto h = cs.eval("(engine:metrics \"query:stable-ref-provenance-sv-stats\")");
    ...
    // (back-compat — #641 doesn't disturb them).
    {
        ...
        CHECK(s_631.has_value(),
              "(engine:metrics \"query:stable-ref-provenance-sv-stats-hash\") reachable (#631 "
    ...
    // the historical #631 primitive that does have `-hash`.
    {
        ...
        auto new_p = cs.eval("(engine:metrics \"query:stable-ref-provenance-sv-stats\")");
        auto old_p = cs.eval("(engine:metrics \"query:stable-ref-provenance-sv-stats-hash\")");
        CHECK(new_p.has_value(),
              "new primitive (engine:metrics \"query:stable-ref-provenance-sv-stats\") reachable "
              "(no -hash suffix)");
        CHECK(old_p.has_value(), "historical primitive (engine:metrics "
                                 "\"query:stable-ref-provenance-sv-stats-hash\") still reachable");
    ...
    // scaffolding atomics.
    {
        ...
        auto worker = [&] {
            for (int i = 0; i < k_iters; ++i) {
                ...
                auto r = cs.eval("(engine:metrics \"query:stable-ref-provenance-sv-stats\")");
                if (r.has_value())
