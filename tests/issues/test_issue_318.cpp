...
bool test_coverage_holes_returns_nodeids() {
    ...
    CHECK(r_defs.has_value(), "(query:where :node-type \"Define\") runs cleanly");
    ...
    CHECK(r_parse.has_value(), "(verify:parse-coverage-feedback \"0\\n1\\n2\\n\") runs");
    ...
    CHECK(r_holes.has_value(), "(verify:coverage-holes) returns cleanly after parse");
    // The combined primitive — pass a report string inline.
    auto r_inline = cs.eval("(verify:coverage-holes \"10\\n11\\n12\\n\")");
    CHECK(r_inline.has_value(), "(verify:coverage-holes) with inline report returns cleanly");
    return true;
}
...
bool test_suggest_constraint_refine_alias() {
    ...
    CHECK(r2.has_value(), "(verify:suggest-constraint-refine) runs cleanly after marking");
...
bool test_composes_with_mutate_query_and_replace() {
    ...
    auto r_mutate = cs.eval("(mutate:query-and-replace "
                            "  (query:where :node-type \"Define\") "
                            "  \"new_value\" "
                            "  \"issue-318 test refine\")");
    CHECK(r_mutate.has_value(), "(mutate:query-and-replace) succeeds after verify:coverage-holes");
    ...
    CHECK(r_snap.has_value(), "(ast:snapshot) runs cleanly after verify+mutate");
    return true;
}
...
bool test_perf_bound_and_docs() {
    ...
    CHECK(us1 < max_us, "(verify:coverage-holes) < 50ms on 200-node workspace");
    CHECK(us2 < max_us, "(verify:suggest-constraint-refine) < 50ms on 200-node workspace");
    return true;
}
...
int aura_issue_318_run() {
    return aura_issue_318_detail::run_tests();
}

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() {
    return aura_issue_318_run();
}
#endif