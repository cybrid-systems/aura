...
static aura::ir::IRFunction make_const_i64_callee(const std::string& name,
                                                  std::uint32_t result_slot, int64_t value) {
    ...
    func.blocks[0].instructions = {
        {aura::ir::IROpcode::ConstI64,
         {result_slot, static_cast<std::uint32_t>(value & 0xFFFFFFFF),
          static_cast<std::uint32_t>((value >> 32) & 0xFFFFFFFF), 0},
         0,
         1},
...
bool test_inline_trivial_callee() {
    ...
    block.instructions = {
        ...
        {aura::ir::IROpcode::Call,
         {0, 0, 0, 1},
         0,
         0}, // call(closure=0, arg_base=0, count=0, result=1)
    };
    ...
    CHECK(inliner.inlined_count() == 1, "inlined_count() == 1 (one substitution)");
    return true;
}
...
bool test_skip_non_trivial() {
    ...
    CHECK(inliner.inlined_count() == 0, "inlined_count() == 0 (no substitution)");
    return true;
}
...
bool test_skip_recursion() {
    ...
    CHECK(inliner.inlined_count() == 0, "inlined_count() == 0 (recursion guard prevented inline)");
    return true;
}
...
bool test_skip_dynamic_callee() {
    ...
    CHECK(inliner.inlined_count() == 0, "inlined_count() == 0 (no static callee info)");
    return true;
}
...
bool test_multiple_call_sites() {
    ...
    CHECK(block.instructions[1].operands[0] == 1, "first inline writes to slot 1");
    CHECK(block.instructions[2].opcode == aura::ir::IROpcode::ConstI64,
          "second Call inlined to ConstI64");
    CHECK(block.instructions[2].operands[0] == 2, "second inline writes to slot 2");
    CHECK(inliner.inlined_count() == 2, "inlined_count() == 2 (two substitutions)");
    return true;
}
...
bool test_tco_tail_call_rewrite() {
    ...
    CHECK(block.instructions[0].opcode == aura::ir::IROpcode::MakeClosure, "MakeClosure preserved");
    CHECK(block.instructions[1].opcode == aura::ir::IROpcode::Jump, "tail Call rewritten to Jump");
    CHECK(block.instructions[1].operands[0] == 0, "Jump target = callee's entry block id (0)");
...
bool test_tco_idempotent() {
    ...
    tco.run(mod);
    CHECK(tco.tco_count() == 0, "second run: 0 TCO (no remaining Call+Return pattern)");
    return true;
}
...
bool test_tco_arg_base_nonzero_single() {
    ...
    CHECK(block.instructions.size() == 4, "arg_base=5 single arg: 4 instructions (was 4: "
                                          "MakeClosure + ConstI64 + Local-insert + Jump)");
    CHECK(block.instructions[0].opcode == aura::ir::IROpcode::MakeClosure, "MakeClosure preserved");
    CHECK(block.instructions[1].opcode == aura::ir::IROpcode::ConstI64,
          "ConstI64 (load arg) preserved");
    CHECK(block.instructions[2].opcode == aura::ir::IROpcode::Local, "TCO inserted Local copy");
    CHECK(block.instructions[2].operands[0] == 0, "Local writes to callee param slot 0");
    CHECK(block.instructions[2].operands[1] == 5, "Local reads from caller arg_base slot 5");
    CHECK(block.instructions[3].opcode == aura::ir::IROpcode::Jump, "Call rewritten to Jump");
    CHECK(block.instructions[3].operands[0] == 0, "Jump target = callee entry block 0");
    CHECK(tco.tco_count() == 1, "tco_count() == 1");
    return true;
}
...
bool test_tco_arg_base_nonzero_multi() {
    ...
    CHECK(block.instructions[4].opcode == aura::ir::IROpcode::Local, "[4] TCO-inserted Local");
    CHECK(block.instructions[4].operands[0] == 0 && block.instructions[4].operands[1] == 5,
          "[4] Local(0, 5) — param 0 from caller slot 5");
    CHECK(block.instructions[5].opcode == aura::ir::IROpcode::Local, "[5] TCO-inserted Local");
    CHECK(block.instructions[5].operands[0] == 1 && block.instructions[5].operands[1] == 6,
          "[5] Local(1, 6) — param 1 from caller slot 6");
    CHECK(block.instructions[6].opcode == aura::ir::IROpcode::Local, "[6] TCO-inserted Local");
    ...
    CHECK(block.instructions[7].operands[0] == 0, "[7] Jump target = callee entry block 0");
    CHECK(tco.tco_count() == 1, "tco_count() == 1");
    return true;
}
...
bool test_tco_inter_block_single_pred() {
    ...
    CHECK(b0.instructions.size() == 2, "block 0: 2 instructions (MakeClosure + Jump; unchanged)");
    CHECK(b0.instructions[1].opcode == aura::ir::IROpcode::Jump, "block 0: terminator is Jump");
    ...
    CHECK(b1.instructions[0].operands[0] == 0, "block 1: Jump target = callee entry block 0");
    CHECK(tco.tco_inter_block_count() == 1, "tco_inter_block_count() == 1 (1 pred rewritten)");
...
bool test_tco_inter_block_multi_pred() {
    ...
    CHECK(b2.instructions.size() == 1, "block 2: collapsed to 1 instruction by intra-block TCO");
    CHECK(b2.instructions[0].opcode == aura::ir::IROpcode::Jump, "block 2: collapsed to Jump");
    CHECK(tco.tco_inter_block_count() == 2, "tco_inter_block_count() == 2 (2 preds rewritten)");
    CHECK(tco.tco_count() == 1,
...
bool test_tco_inter_block_tail_call_is_entry() {
    ...
    CHECK(
        b0.instructions.size() == 2,
        "entry block: 2 instructions (MakeClosure + Jump; intra-block TCO collapsed Call+Return)");
    CHECK(b0.instructions[1].opcode == aura::ir::IROpcode::Jump,
          "entry block: tail Call rewritten to Jump");
    CHECK(tco.tco_inter_block_count() == 0, "tco_inter_block_count() == 0 (no preds to rewrite)");
    CHECK(tco.tco_count() == 1, "tco_count() == 1 (intra-block TCO fired on the entry block)");
    return true;
}
...
bool test_tco_inter_block_dynamic_callee_skipped() {
    ...
    CHECK(tco.tco_count() == 0, "tco_count() == 0 (intra-block TCO also skipped dynamic callee)");
    return true;
}
...
bool test_tco_inter_block_coexist_with_intra() {
    ...
    CHECK(a.instructions.size() == 2, "block A: 2 instrs (MakeClosure + Jump; intra-block TCO)");
    ...
    CHECK(b.instructions[1].operands[0] == 0, "block B: Jump target re-targeted to callee entry 0");
    // C (tail-call block): collapsed to Jump by intra-block TCO
    auto& c = mod.functions[2].blocks[2];
    CHECK(c.instructions.size() == 1, "block C: collapsed to 1 instr by intra-block TCO");
    ...
    CHECK(e.instructions.size() == 1, "block E: collapsed to 1 instr by intra-block TCO");
    CHECK(tco.tco_inter_block_count() == 2, "tco_inter_block_count() == 2 (B and D re-targeted)");
...
bool test_inline_mutual_recursion_AB() {
    ...
    CHECK(inliner.inlined_count() == 0, "inlined_count() == 0 (no inlining happened)");
    return true;
}
...
bool test_inline_non_recursive_chain() {
    ...
    CHECK(aCall.opcode == aura::ir::IROpcode::Call,
          "A's Call to B NOT inlined (B's pre-inlined form is 3 instrs; trivial-inlinable requires "
          "2)");
...
bool test_inline_mixed_recursive_and_non_recursive() {
    ...
    auto& aCall = mod.functions[0].blocks[0].instructions[1];
    CHECK(aCall.opcode == aura::ir::IROpcode::Call, "A's Call to B NOT inlined (mutual recursion)");
    // B's Call to A: NOT inlined (mutual recursion)
    auto& bCall = mod.functions[1].blocks[0].instructions[1];
    CHECK(bCall.opcode == aura::ir::IROpcode::Call, "B's Call to A NOT inlined (mutual recursion)");
    ...
    CHECK(cCall.operands[1] == 99, "C's inlined constant is 99 (D's value)");
    CHECK(inliner.inlined_count() == 1, "inlined_count() == 1 (only C→D was inlined; A↔B skipped)");
    return true;
}
...
bool test_inline_three_way_mutual_recursion() {
    ...
    CHECK(inliner.inlined_count() == 0, "inlined_count() == 0 (3-way recursion: all skipped)");
    return true;
}
...
int aura_issue_171_run() {
    return aura_issue_171_detail::run_tests();
}
