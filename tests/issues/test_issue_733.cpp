// test_issue_733.cpp — Issue #733: Macro SyntaxMarker propagation + IR/JIT
// hygiene enforcement observability primitive (non-duplicative with #714
// query:self-evolution-closedloop-stats ref-drift + rollback + feedback-
// mutate-rounds, #455 internal ir marker snapshot, #373 mutate hygiene
// guard flat.is_macro_introduced internal check). #733 tracks the
// *marker propagation + IR/JIT enforcement* specifically across the
// entire compile/execution pipeline (macro expand -> AST -> lowering ->
// IR -> JIT hot-path -> Interpreter) as separate per-decision-point
// counters).
//
// Scope-limited close: the issue body asks for: (1) propagate_marker_from_ast
// helper in lowering_impl.cpp + ensure every IRInstruction created from
// AST node copies flat.marker(node_id) to instr.source_marker (closure
// bodies, PrimCall args, linear ops, cached define paths), (2) ir_soa.ixx
// marker_ column to IRFunctionSoA / instruction columns with cascade on
// mutate, (3) aura_jit.cpp + aura_jit_runtime.cpp + ir_executor.ixx
// conservative policy on source_marker==MacroIntroduced (no aggressive
// deopt-elide, respect hygiene in closure capture, force Interpreter
// fallback or extra epoch check), (4) IRFunction creation in service/
// lowering set func.marker from root AST marker, (5) primitive
// query:ir-marker-hygiene-stats, (6) tests/test_macro_marker_propagation_
// ir_jit_post_mutate.cpp harness. Items (1)/(2)/(3)/(4)/(6) require
// dedicated wiring into lowering_impl.cpp + ir_soa.ixx + aura_jit.cpp +
// aura_jit_runtime.cpp + ir_executor.ixx + new test harness; each is a
// non-trivial focused session.
//
// For this PR we ship:
//
//   1. 5 new atomics in CompilerMetrics:
//        ir_marker_user_instrs_total
//        ir_marker_macro_introduced_instrs_total
//        ir_marker_loss_events_total
//        ir_hygiene_jit_violations_prevented_total
//        ir_hygiene_marker_propagation_hits_total
//   2. 5 new public bump helpers in Evaluator:
//        bump_ir_marker_user_instr
//        bump_ir_marker_macro_introduced_instr
//        bump_ir_marker_loss_event
//        bump_ir_hygiene_jit_violation_prevented
//        bump_ir_hygiene_marker_propagation_hit
//   3. New standalone (query:ir-marker-hygiene-stats, schema 733)
//      primitive exposing the 5 counters (6-entry hash: 5 fields +
//      schema sentinel)
...
//     (ref drift + rollback success + feedback mutate rounds; no marker
//     propagation / IR/JIT enforcement signals)
//   - #455 (ir marker snapshot) — internal mechanics, no observability
//     surface
//   - #373 (mutate hygiene guard) — flat.is_macro_introduced internal
//     check
//   - #733 is the FIRST observability surface that tracks the *marker
//     propagation + IR/JIT enforcement* specifically as separate
//     per-decision-point counters
//
// ACs:
//   AC1: hash shape (5 fields + schema sentinel = 6 entries)
//   AC2: 5 counters == 0 on fresh service
//   AC3: schema == 733 (drift sentinel)
...
//        + #731 + #732 sibling primitives still reachable with
//        their schema sentinels intact
//
// (We do NOT implement propagate_marker_from_ast helper, do NOT
// add ir_soa.ixx marker_ column, do NOT wire aura_jit.cpp /
// aura_jit_runtime.cpp / ir_executor.ixx conservative policy, do
// NOT set IRFunction creation marker from root AST marker, do
// NOT run the tests/test_macro_marker_propagation_ir_jit_post_mutate
// harness — those are the bulk of this issue's remaining scope.)
...
namespace aura_issue_733_detail {
...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println("\n--- AC1: (engine:metrics \"query:ir-marker-hygiene-stats\") hash shape ---");
    auto r = cs.eval("(engine:metrics \"query:ir-marker-hygiene-stats\")");
    CHECK(r && aura::compiler::types::is_hash(*r),
          "(engine:metrics \"query:ir-marker-hygiene-stats\") returns a hash");
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto user =
        hash_int_field(cs, "(engine:metrics \"query:ir-marker-hygiene-stats\")", "user-instrs");
    CHECK(user == 0, std::format("user-instrs = {} (expected 0 on fresh service)", user));
    const auto macro = hash_int_field(cs, "(engine:metrics \"query:ir-marker-hygiene-stats\")",
                                      "macro-introduced-instrs");
    CHECK(macro == 0,
          std::format("macro-introduced-instrs = {} (expected 0 on fresh service)", macro));
    const auto loss = hash_int_field(cs, "(engine:metrics \"query:ir-marker-hygiene-stats\")",
                                     "marker-loss-events");
    CHECK(loss == 0, std::format("marker-loss-events = {} (expected 0 on fresh service)", loss));
    const auto jit_h = hash_int_field(cs, "(engine:metrics \"query:ir-marker-hygiene-stats\")",
                                      "jit-hygiene-violations-prevented");
    CHECK(
        jit_h == 0,
        std::format("jit-hygiene-violations-prevented = {} (expected 0 on fresh service)", jit_h));
    const auto prop = hash_int_field(cs, "(engine:metrics \"query:ir-marker-hygiene-stats\")",
                                     "marker-propagation-hits");
    CHECK(prop == 0,
          std::format("marker-propagation-hits = {} (expected 0 on fresh service)", prop));
}
...
static void run_ac3_schema_sentinel(aura::compiler::CompilerService& cs) {
    ...
    CHECK(schema == 733, std::format("schema = {} (expected 733)", schema));
}
...
static void run_ac4_bump_accessible(aura::compiler::CompilerService& cs) {
    ...
    // surface. These helpers exist so future lowering_impl.cpp +
    ...
    ev.bump_ir_marker_user_instr();
    ev.bump_ir_marker_user_instr();
    ev.bump_ir_marker_user_instr();
    ev.bump_ir_marker_user_instr();
    ev.bump_ir_marker_macro_introduced_instr();
    ev.bump_ir_marker_macro_introduced_instr();
    ev.bump_ir_marker_macro_introduced_instr();
    ev.bump_ir_marker_macro_introduced_instr();
    ev.bump_ir_marker_macro_introduced_instr();
    ev.bump_ir_marker_loss_event();
    ev.bump_ir_marker_loss_event();
    ev.bump_ir_hygiene_jit_violation_prevented();
    ev.bump_ir_hygiene_jit_violation_prevented();
    ev.bump_ir_hygiene_jit_violation_prevented();
