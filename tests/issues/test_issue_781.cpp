...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println(
        "\n--- AC1: (engine:metrics \"query:zero-copy-framebuffer-stats\") hash shape ---");
    auto r = cs.eval("(engine:metrics \"query:zero-copy-framebuffer-stats\")");
    CHECK(r && aura::compiler::types::is_hash(*r),
          "(engine:metrics \"query:zero-copy-framebuffer-stats\") returns a hash");
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto pair_alloc = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "pair-alloc-total");
    ...
    const auto zero_copy = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "zero-copy-supported");
    CHECK(
        zero_copy == 1,
        std::format("zero-copy-supported = {} (expected 1 — Phase 1 zero_copy_output)", zero_copy));
    const auto ansi_helper = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "ansi-helper-supported");
    CHECK(
        ansi_helper == 1,
        std::format("ansi-helper-supported = {} (expected 1 — Phase 1 ANSI helpers)", ansi_helper));
    const auto mem_prof = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "memory-profiling-supported");
    CHECK(mem_prof == 1,
    ...
    const auto rec = hash_int_field(cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")",
                                    "recommendation");
...
static void run_ac4_production_path_bumps_and_benchmark(aura::compiler::CompilerService& cs) {
    ...
    const auto initial_pair_alloc = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "pair-alloc-total");
    ...
    const auto after_pair_alloc = hash_int_field(
        cs, "(engine:metrics \"query:zero-copy-framebuffer-stats\")", "pair-alloc-total");
