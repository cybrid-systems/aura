...
static std::string aura_eval_str(aura::compiler::CompilerService& cs, const std::string& src) {
    auto r = cs.eval(src);
    if (!r)
        return "<error>";
    return "<value>";
...
static bool eval_is_true(aura::compiler::CompilerService& cs, const std::string& src) {
    auto r = cs.eval(src);
    if (!r)
        return false;
...
bool test_bitvec_explicit_width() {
    ...
    CHECK(eval_is_true(cs, "(require \"std/eda\" all:)"
                           "(eda:bitvec? (make-eda:bitvec 8))"),
          "BitVec predicate accepts explicit-width BitVec");
    CHECK(eval_is_true(cs, "(= (eda:bitvec-width (make-eda:bitvec 16)) 16)"),
...
bool test_bitvec_compat_equal() {
    ...
    CHECK(eval_is_true(cs, "(eda:bitvec-compatible? (make-eda:bitvec-32) (make-eda:bitvec-32))"),
...
bool test_bitvec_compat_mismatch() {
    ...
    CHECK(
        eval_is_true(cs, "(not (eda:bitvec-compatible? (make-eda:bitvec-8) (make-eda:bitvec-64)))"),
...
bool test_clock_domain_compat() {
    ...
    CHECK(eval_is_true(cs, "(not (eda:clock-domain-compatible? "
                           "  (make-eda:clock-domain 'fast) "
                           "  (make-eda:clock-domain 'slow)))"),
          "Different clock domain names → not compatible");
    CHECK(eval_is_true(cs, "(eda:clock-domain? eda:default-clock-domain)"),
...
bool test_reset_type_compat() {
    ...
    CHECK(eval_is_true(cs, "(not (eda:reset-type-compatible? "
                           "  (make-eda:reset-type 'async) "
...
int aura_issue_228_run() {
    return aura_issue_228_detail::run_tests();
}
