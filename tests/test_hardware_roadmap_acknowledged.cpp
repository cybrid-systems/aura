...
bool test_roadmap_doc_present() {
    std::println("\n--- AC1: docs/ROADMAP-HARDWARE.md present ---");
    std::string content = read_file("/home/dev/code/aura/docs/ROADMAP-HARDWARE.md");
    CHECK(!content.empty(), "docs/ROADMAP-HARDWARE.md exists on disk");
    std::println("  doc length: {} chars", content.size());
    CHECK(content.size() > 1000, "roadmap doc has substantive content (>= 1000 chars)");
    return true;
}
...
bool test_roadmap_phases_present() {
    ...
    std::println("  phases: Phase 0={} Phase 1={} Phase 2={}", has_phase0, has_phase1, has_phase2);
...
bool test_roadmap_required_sections() {
    ...
    std::string content = read_file("/home/dev/code/aura/docs/ROADMAP-HARDWARE.md");
    const bool has_304_ref = content.find("Issue #304") != std::string::npos;
    const bool has_risks =
        content.find("Risks") != std::string::npos || content.find("Risk") != std::string::npos;
    const bool has_success = content.find("Success metric") != std::string::npos;
    const bool has_cross_refs = content.find("Cross-references") != std::string::npos;
    const bool has_decision_doc =
        content.find("primitive-vs-stdlib-decision-framework.md") != std::string::npos;
    std::println("  required sections: 304_ref={} risks={} success={} "
                 "cross_refs={} decision_doc={}",
                 has_304_ref, has_risks, has_success, has_cross_refs, has_decision_doc);
    ...
    CHECK(has_decision_doc, "doc cross-references the decision framework");
    return true;
}
...
bool test_roadmap_status_table() {
    ...
    const bool has_pending = content.find("child issue pending") != std::string::npos;
    std::println("  Status table: status={} tbd={} pending={}", has_status, has_tbd, has_pending);
...
int aura_issue_304_run() {
    return aura_issue_304_detail::run_tests();
}

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() {
    return aura_issue_304_run();
}
#endif