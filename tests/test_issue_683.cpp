...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (query:linear-ownership-gc-stats) '{}')", key));
...
int main() {
    ...
    // AC1: stats hash fields
    {
        ...
        CHECK(stat_int(cs, "safepoint-violations") >= 0,
              "safepoint-violations present");
        CHECK(stat_int(cs, "steal-enforced") >= 0, "steal-enforced present");
        CHECK(stat_int(cs, "relower-revalidate-hits") >= 0,
              "relower-revalidate-hits present");
    }
    ...
    // AC3: mutate:rebind triggers re-lower revalidate probe
    {
        ...
        CHECK(relower_after > relower_before,
              std::format("relower-revalidate-hits grew ({} -> {})",
    ...
    // AC4: fiber steal probe + stress path
    {
        ...
        CHECK(steal_after_probe > steal_before,
              std::format("steal-enforced grew after transfer ({} -> {})",
                          steal_before, steal_after_probe));
