...
static std::string get_workspace_source(CompilerService& cs) {
    ...
    if (!aura::compiler::types::is_string(*r)) return "<not-string>";
    auto idx = aura::compiler::types::as_string_idx(*r);
    if (idx >= cs.evaluator().string_heap().size()) return "<bad-idx>";
...
bool test_successful_commit_changes_source() {
    ...
    std::println("  pre-state: cells={} pairs={} string_heap={} source='{}'",
                 cells_before, pairs_before, string_heap_before, source_before);
    ...
    std::println("  post-state: cells={} pairs={} string_heap={} source='{}'",
                 cells_after, pairs_after, string_heap_after, source_after);
    // Assertion 1: source contains the new value '2'.
    CHECK(source_after.find("2") != std::string::npos,
          "post-commit source contains new value '2'");
    // Assertion 2: cells size bounded (no dramatic growth).
    CHECK(cells_after <= cells_before + 5,
          "cells_.size() bounded growth after commit");
    // Assertion 3: pairs size bounded.
    CHECK(pairs_after <= pairs_before + 10,
          "pairs_.size() bounded growth after commit");
...
bool test_auto_rollback_on_failure() {
    ...
    std::size_t cells_before = cs.evaluator().cells().size();
    std::size_t pairs_before = cs.evaluator().pairs().size();
    std::println("  pre-state: cells={} pairs={} source='{}'",
                 cells_before, pairs_before, source_before);
    cs.evaluator().set_auto_rollback_on_panic(true);
    ...
    std::size_t cells_after = cs.evaluator().cells().size();
    std::size_t pairs_after = cs.evaluator().pairs().size();
    std::println("  post-failure: cells={} pairs={} source='{}'",
                 cells_after, pairs_after, source_after);
    ...
    CHECK(cells_after == cells_before,
          "cells_.size() unchanged after failed mutate (rollback)");
    CHECK(pairs_after <= pairs_before + 16,
          "pairs_.size() bounded growth on failed eval");
    return true;
}
...
bool test_auto_rollback_toggle() {
    ...
    // Toggle on.
    cs.evaluator().set_auto_rollback_on_panic(true);
    CHECK(cs.evaluator().auto_rollback_on_panic() == true,
          "auto-rollback toggle ON works");
    // Toggle back off.
    cs.evaluator().set_auto_rollback_on_panic(false);
    CHECK(cs.evaluator().auto_rollback_on_panic() == false,
          "auto-rollback toggle OFF works");
    return true;
}
