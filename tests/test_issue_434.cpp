...
bool test_initial_counter_zero() {
    ...
    CHECK_EQ(snap.narrowing_dirty_recovery_total, 0u, "narrowing_dirty_recovery_total == 0");
    return true;
}
...
bool test_mutate_narrowed_var_bumps_dirty_recovery() {
    ...
    auto snap = cs.snapshot();
    std::println("  narrowing_dirty_recovery_total: {}", snap.narrowing_dirty_recovery_total);
