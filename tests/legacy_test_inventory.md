...
**Generated:** 2026-09-26 by `scripts/tools/inventory_legacy_tests.py`
...
- Existing `*_batch` drivers (migration milestones): **101**
...
- `tests/compiler/test_atomic_batch_rebind_hygiene_lockless.cpp` → theme `mutation_dirty`
...
- `tests/compiler/test_abort_ir_cache_fence_first.cpp`
...
- `tests/core/test_arena_required_cover_no_value_only.cpp`
...
- `tests/compiler/test_ast_unparse.cpp`
...
- `tests/compiler/test_atomic_batch_rebind_hygiene_lockless.cpp`
...
- `tests/compiler/test_capability_macro_self_evo_reason_uniformity.cpp`
...
- `tests/core/test_compact_nodes_provenance_schema_remap.cpp`
...
- `tests/compiler/test_compiler_hot_update_facade.cpp`
...
- `tests/compiler/test_dead_coercion_decision_invalidate.cpp`
...
- `tests/compiler/test_incremental_facade_dirty_names_snapshot.cpp`
...
- `tests/compiler/test_ir_typed_entry_proof_authority.cpp`
...
- `tests/compiler/test_jit_prim_dispatch_telemetry.cpp`
...
- `tests/compiler/test_macro_clone_target_atomicity.cpp`
...
- `tests/compiler/test_macro_inner_expand_marker.cpp`
...
- `tests/compiler/test_occurrence_persist_rehydrate.cpp`
...
- `tests/compiler/test_pack_pipeline_strict.cpp`
...
- `tests/compiler/test_partial_relower_impact_production.cpp`
...
- `tests/compiler/test_peer_jit_name_soft_stale.cpp`
...
- `tests/compiler/test_replace_pattern_allow_macro_unify.cpp`
...
- `tests/compiler/test_rest_param_hygiene_eval_flat.cpp`
...
- `tests/compiler/test_scalar_mutate_record_patch_hygiene.cpp`
...
- `tests/compiler/test_typed_audit_commit_readiness_live_policy.cpp`
- `tests/compiler/test_typed_audit_commit_readiness_recover_acl.cpp`
...
- `tests/compiler/test_grant_epoch_retain_restricted.cpp` (—) [large, domain_suite, theme_compiler] — Issue #3774 — MSE production session rows arm live_session_grants.
...
- `tests/core/test_panic_checkpoint_aba.cpp` (—) [domain_suite, theme_core] — tests/core/test_panic_checkpoint_aba.cpp
...
- `tests/compiler/test_atomic_batch_rebind_hygiene_lockless.cpp` (—) [batch_driver, domain_suite, theme_compiler] — AC1: source cites the new-body walk in eval_flat_apply_mutate_rebind
...
- `tests/core/test_check_and_record_wildcard_strip.cpp` (—) [domain_suite, theme_core] — tests/core/test_check_and_record_wildcard_strip.cpp
...
- `tests/compiler/test_incremental_facade_dirty_names_snapshot.cpp` (—) [domain_suite, theme_compiler] — Fix contract (AC1–AC6 from the issue body):
...
- `tests/compiler/test_module_rebind_residual.cpp` (—) [domain_suite, theme_compiler] — Issue #4109 — live-body behind captures are not copied; the
...
- `tests/compiler/test_occurrence_persist_rehydrate.cpp` (—) [domain_suite, theme_compiler] — AC1: Production mode \u2014 mutate \u2192 expected_fp stays 0
...
- `tests/compiler/test_soa_dirty_aware_pipeline.cpp` (—) [large, domain_suite, theme_compiler] — Issue #2907 — sunset SoAtoAoSBridgePass; production SoA dirty hot pack.
...
- `tests/compiler/test_stable_ref_provenance_batch.cpp` (—) [large, batch_driver, domain_suite, theme_compiler] — tests/compiler/test_stable_ref_provenance_batch.cpp — test_stable_ref 3-merge (R19 phase 20).
- `tests/serve/test_stable_ref_provenance_fiber_cow.cpp` (—) [large, domain_suite, theme_serve] — test_stable_ref_provenance_fiber_cow.cpp — Merged #457/#497/#527/#540/#549 + #551/#552 (#1978).
- `tests/core/test_stable_ref_tenant_capture.cpp` (—) [large, domain_suite, theme_core] — atomic-batch pin (#2073). Issue #2960: children_stable / parent_stable are
...
- `tests/serve/test_production_sweep.cpp` (—) [domain_suite, theme_serve] — test_production_sweep.cpp — fiber production sweep (standalone; SIGSEGV in batch)
...
- `tests/orch/test_scope_join_tree_visibility.cpp` (—) [domain_suite, theme_orch] — tests/orch/test_scope_join_tree_visibility.cpp
- `tests/orch/test_security_schedule_gate.cpp` (—) [large, domain_suite, theme_orch] — tests/orch/test_security_schedule_gate.cpp
- `tests/serve/test_steal_complete_gc_defer.cpp` (—) [large, domain_suite, theme_serve] — AC1: try_steal_from success always invokes aura_evaluator_on_steal_complete
...
### `linear_ownership` — Linear ownership / borrow / consume (26)
...
### `edsl_hygiene` — EDSL / macro hygiene / reflect (60)
...
- `tests/compiler/test_macro_hygiene_limits.cpp` (—) [large, domain_suite, theme_compiler] — AC1: lower runtime cap → expand at depth=cap+1 clamps/NULL_NODE + metric;
...
- `tests/compiler/test_scalar_mutate_record_patch_hygiene.cpp` (—) [domain_suite, theme_compiler] — AC1: source cites #3131 in evaluator_primitives_mutate.cpp —
...
- `tests/compiler/test_cascade_relower_silent_skip.cpp` (—) [large, domain_suite, theme_compiler] — AC1: cascade cites #2813; skipped/ran metrics; warn path
...
- `tests/compiler/test_hot_update_relower_success_coverage.cpp` (—) [large, domain_suite, theme_compiler] — Fix contract (AC1–AC5 from the issue body):
...
- `tests/compiler/test_issue_3096.cpp` (#3096) [domain_suite, theme_compiler] — min-dirty / coverage-verify pass via maybe_coverage_verify_min_dirty
...
- `tests/compiler/test_jit_prim_dispatch_telemetry.cpp` (—) [domain_suite, theme_compiler] — AC1: Restricted+MT production face, PrimMeta-only Mutate stub over a
...
- `tests/compiler/test_tco_arg_base_oob.cpp` (—) [domain_suite, theme_compiler] — AC1: source bounds-checks arg_base+count vs local_count; cites #2832
...
- `tests/compiler/test_ast_unparse.cpp` (—) [domain_suite, theme_compiler] — AC1: unparse callable via aura.core.ast_unparse (no Evaluator import)
...
- `tests/compiler/test_query_find_by_define.cpp` (—) [domain_suite, theme_compiler] — tests/compiler/test_query_find_by_define.cpp --
...
- `tests/compiler/test_live_closure_full_restamp.cpp` (—) [large, domain_suite, theme_compiler] — AC1: N named closures + reemit → epoch_restamp_total ≥ N
...
- `tests/compiler/test_security_posture_trail.cpp` (—) [large, domain_suite, theme_compiler] — AC1: posture returns schema-2534
...
- `tests/core/test_grant_effect_wildcard_write_fence.cpp` (—) [domain_suite, theme_core] — tests/core/test_grant_effect_wildcard_write_fence.cpp
...
- `tests/compiler/test_pack_pipeline_strict.cpp` (—) [domain_suite, theme_compiler] — face lives in test_tree_walker_fallback_strict.cpp (sandbox=off →
