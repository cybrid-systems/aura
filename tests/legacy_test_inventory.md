...
**Generated:** 2026-07-27 by `scripts/inventory_legacy_tests.py`
...
| `tests/core/test_*.cpp` | 453 | Preferred destination suites |
| **Total scanned** | **453** | |
...
|-------|-------|-------:|-----:|-------:|------:|--------------------|
| `arena_compaction` | Arena / compaction / GC | 0 | 0 | 49 | 49 | P0 — well-contained, batch drivers already exist |
| `mutation_dirty` | Mutation / dirty propagation / provenance | 0 | 0 | 128 | 128 | P0 — high volume; strong domain suite foothold |
| `fiber_orch` | Fiber / orchestration / steal / Guard | 0 | 0 | 43 | 43 | P1 — domain suite already collapses many obs gates |
| `linear_ownership` | Linear ownership / borrow / consume | 0 | 0 | 10 | 10 | P1 — small, already partially batched |
| `edsl_hygiene` | EDSL / macro hygiene / reflect | 0 | 0 | 25 | 25 | P1 — domain hygiene suite exists |
...
- `tests/compiler/test_zero_copy_present_default_2135.cpp`
...
### `arena_compaction` — Arena / compaction / GC (49)
...
- `tests/orch/test_agent_scope_2083.cpp` (#2083) [domain_suite, theme_orch] — test_agent_scope_2083.cpp — Issue #2083 AgentScope opt-in + #2161 watch_all.
...
### `mutation_dirty` — Mutation / dirty propagation / provenance (128)
...
### `linear_ownership` — Linear ownership / borrow / consume (10)
...
### `edsl_hygiene` — EDSL / macro hygiene / reflect (25)
...
- `tests/compiler/test_macro_fiber_hygiene.cpp` (—) [domain_suite, theme_compiler] — AC1: source cites #2097 + FiberHygieneStats + get_fiber_hygiene_metrics
...
### `jit_incremental` — JIT / AOT / incremental relower (45)
...
### `observability` — Observability / metrics / query:*-stats (105)
...
- `tests/compiler/test_dep_graph_hybrid_cascade_2110.cpp` (#2110) [domain_suite, theme_compiler] — AC1: record_dependency populates both string graph and NodeId mirror
...
- `tests/compiler/test_render_strategy_layer_2138.cpp` (#2138) [domain_suite, theme_compiler] — AC1: strategy can be set/get; present path consults it
