...
static int build_constraint_workspace(
    aura::compiler::CompilerService& cs, int n_constraints) {
...
static bool mark_constraint_dirty(
    aura::compiler::CompilerService& cs, int constraint_idx) {
    std::string report = std::to_string(constraint_idx) + "\n";
    return cs.eval(
        std::string("(verify:parse-coverage-feedback \"") + report + "\")")
        .has_value();
}
...
// failure.
static long long count_coverage_dirty(
...
static bool refine_constraint(
    aura::compiler::CompilerService& cs, int constraint_idx) {
    std::string expr = std::string("(define constraint-") +
                       std::to_string(constraint_idx) +
                       " (* 2 " +
                       std::to_string(constraint_idx) + "))";
    return cs.eval(
        std::string("(mutate:query-and-replace (query:defines-by-marker "
                    "\"User\") \"") + expr + "\" \"refine constraint-" +
        std::to_string(constraint_idx) + "\")").has_value();
}

// Read (query:defines) length — used to verify rollback
// didn't lose any nodes.
static long long count_defines(
    aura::compiler::CompilerService& cs) {
...
bool test_full_closed_loop_runs() {
    ...
    CHECK(build_constraint_workspace(cs, 3) == 3,
          "3 constraints built");
    // Step 2: snapshot the pre-iteration state.
    auto snap = cs.eval("(ast:snapshot)");
    CHECK(snap.has_value() && aura::compiler::types::is_int(*snap),
          "snapshot created");
    ...
    for (int i = 0; i < 3; ++i) {
        // (a) mark dirty
        if (!mark_constraint_dirty(cs, i)) { ++g_failed; return false; }
        // (b) list holes
        auto holes = cs.eval("(verify:coverage-holes)");
        if (!holes) { ++g_failed; return false; }
        // (c) refine
        if (!refine_constraint(cs, i)) { ++g_failed; return false; }
    }
...
bool test_rollback_safety() {
    ...
    CHECK(snap.has_value() && aura::compiler::types::is_int(*snap),
          "snapshot created");
    if (!snap || !aura::compiler::types::is_int(*snap)) return false;
    const auto snap_id = static_cast<std::int64_t>(
        aura::compiler::types::as_int(*snap));
    // Record pre-iteration define count.
    const auto pre_define_count = count_defines(cs);
    CHECK(pre_define_count == 3,
          "pre-iteration: 3 defines");
    ...
    CHECK(post_mutation_count == 3,
          "post-mutation: 3 defines (no dangling refs)");
    // Rollback.
    auto rb = cs.eval(std::string("(ast:restore ") +
    ...
    CHECK(post_rollback_count == 3,
          "post-rollback: 3 defines (snapshot restored)");
    return true;
}
...
bool test_end_to_end_aura_only() {
    ...
    auto r5 = cs.eval(
        "(mutate:query-and-replace (query:defines-by-marker \"User\") "
        "\"(define c-0 (* 2 0))\" \"ai-refine-iteration-1\")");
    CHECK(r5.has_value(),
          "mutate:query-and-replace refines c-0");
...
int aura_issue_319_run() { return aura_issue_319_detail::run_tests(); }

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() { return aura_issue_319_run(); }
#endif