...
BENCHMARKS = [
    ...
    BenchCase("letrec_fact", "(letrec ((fact (lambda (n) (if (= n 0) 1 (* n (fact (- n 1))))))) (fact 10))", "eval", expected_val=3628800),
    BenchCase("fib_20", "(letrec ((fib (lambda (n) (if (< n 2) n (+ (fib (- n 1)) (fib (- n 2))))))) (fib 20))", "eval", expected_val=6765),

    # ── L4: Lambda / Closure ─────────────────────────────────
    BenchCase("lambda_apply", "((lambda (x) (* x 2)) 5)", "eval", expected_val=10),
    BenchCase("closure", "(let ((f (lambda (x) (+ x 1)))) (f 41))", "eval", expected_val=42),
    BenchCase("higher_order", "((lambda (f) (f 10)) (lambda (x) (* x x)))", "eval", expected_val=100),
    ...
    BenchCase("tc_string_append", '(string-append "a" "b")', "typecheck", expected_type="String"),
    ...
    BenchCase("tc_occurrence", '(let ((x "hello")) (if (string? x) x "fallback"))', "typecheck", expected_type="String"),
...
def run_all() -> BenchSuiteResult:
    ...
    print(f"{'─'*60}")
    print(f"  Total: {suite.total_cases} cases, "
