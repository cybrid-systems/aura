...
// (engine:metrics \"query:shape-pass-hotpath-contracts-stats\")
...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println(
        "\n--- AC1: (engine:metrics \"query:shape-pass-hotpath-contracts-stats\") hash shape ---");
    auto r = cs.eval("(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")");
    CHECK(r && aura::compiler::types::is_hash(*r),
          "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\") returns a hash");
    ...
    for (const auto& k : keys) {
        auto f = cs.eval(std::format(
            "(hash-ref (engine:metrics \"query:shape-pass-hotpath-contracts-stats\") '{}')", k));
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto soa_view =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
    ...
    CHECK(on_compact == 0,
          std::format("on-compact-hook-invocations-total = {} (expected 0 on fresh "
                      ...
                      on_compact));
    const auto concepts = hash_int_field(
        cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")", "concepts-active");
    CHECK(concepts == 0,
    ...
    const auto rec = hash_int_field(
        cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")", "recommendation");
    CHECK(rec == 3,
...
static void run_ac4_bump_correctness(aura::compiler::CompilerService& cs) {
    ...
    // Snapshot before.
    const auto soa_view_before =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "soa-view-violations-caught-total");
    const auto shape_stable_before =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "shape-stable-pass-violations-total");
    const auto targeted_deopt_before =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "targeted-deopt-via-impact-scope-total");
    const auto on_compact_before =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "on-compact-hook-invocations-total");
    ...
    const auto soa_view_after =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "soa-view-violations-caught-total");
    const auto shape_stable_after =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "shape-stable-pass-violations-total");
    const auto targeted_deopt_after =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "targeted-deopt-via-impact-scope-total");
    const auto on_compact_after =
        hash_int_field(cs, "(engine:metrics \"query:shape-pass-hotpath-contracts-stats\")",
                       "on-compact-hook-invocations-total");
...
static void run_ac5_sibling_regression(aura::compiler::CompilerService& cs) {
    ...
    auto a768 = cs.eval("(engine:metrics \"query:shape-pass-hotpath-stats\")");
    auto a794 = cs.eval("(engine:metrics \"query:full-closedloop-compiler-edsl-fidelity-stats\")");
    ...
    const auto a794_schema = hash_int_field(
