...
static void run_ac1_shape(aura::compiler::CompilerService& cs) {
    std::println(
        "\n--- AC1: (engine:metrics \"query:envframe-dualpath-policy-stats\") hash shape ---");
    auto r = cs.eval("(engine:metrics \"query:envframe-dualpath-policy-stats\")");
    CHECK(r && aura::compiler::types::is_hash(*r),
          "(engine:metrics \"query:envframe-dualpath-policy-stats\") returns a hash");
    ...
    for (const auto& k : keys) {
        auto f = cs.eval(std::format(
            "(hash-ref (engine:metrics \"query:envframe-dualpath-policy-stats\") '{}')", k));
...
static void run_ac2_fresh_zero(aura::compiler::CompilerService& cs) {
    ...
    const auto panic = hash_int_field(
        cs, "(engine:metrics \"query:envframe-dualpath-policy-stats\")", "desync-panic-count");
    CHECK(panic == 0, std::format("desync-panic-count = {} (expected 0 on fresh service)", panic));
    const auto gc_stale = hash_int_field(
        cs, "(engine:metrics \"query:envframe-dualpath-policy-stats\")", "gc-stale-desync-hits");
    CHECK(gc_stale == 0,
          std::format("gc-stale-desync-hits = {} (expected 0 on fresh service)", gc_stale));
    const auto repair = hash_int_field(
        cs, "(engine:metrics \"query:envframe-dualpath-policy-stats\")", "dualpath-repair");
    CHECK(repair == 0, std::format("dualpath-repair = {} (expected 0 on fresh service)", repair));
    const auto mismatch = hash_int_field(
        cs, "(engine:metrics \"query:envframe-dualpath-policy-stats\")", "version-mismatch");
    CHECK(mismatch == 0,
