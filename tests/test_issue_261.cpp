...
bool test_recycle_primitive() {
    ...
    if (!cs.eval("(set-code \"(define x 1)\")")) { ++g_failed; return false; }
...
bool test_compact_primitive() {
    ...
    aura::compiler::CompilerService cs;
    if (!cs.eval("(set-code \"(begin (define x 1) (define y 2))\")")) { ++g_failed; return false; }
...
bool test_snapshot_restore_hook() {
    ...
    aura::compiler::CompilerService cs;
    if (!cs.eval("(set-code \"(define x 42)\")")) { ++g_failed; return false; }
...
int aura_issue_261_run() { return aura_issue_261_detail::run_tests(); }

#ifndef AURA_ISSUE_BUNDLE_MEMBER
int main() { return aura_issue_261_run(); }
#endif