...
static std::int64_t stat_int(aura::compiler::CompilerService& cs, std::string_view key) {
    auto r = cs.eval(std::format(
        "(hash-ref (query:primitives-extension-stats) '{}')", key));
    if (!r || !aura::compiler::types::is_int(*r))
        return -1;
    return aura::compiler::types::as_int(*r);
}

static std::string hash_str(aura::compiler::CompilerService& cs,
                            const char* hash_expr,
                            std::string_view key) {
...
int main() {
    ...
    // AC1: query:primitives-extension-stats hash fields
    {
        ...
        CHECK(stat_int(cs, "category-sva") >= 2,
              "category-sva >= 2 (weaken + coverpoint)");
        CHECK(stat_int(cs, "category-verification") >= 1,
              "category-verification >= 1");
        CHECK(stat_int(cs, "category-eda") >= 1,
              "category-eda >= 1");
        CHECK(stat_int(cs, "documented-with-schema") >= 4,
              "documented-with-schema >= 4");
        CHECK(stat_int(cs, "extension-kit-version") == 1,
              "extension-kit-version == 1");
        CHECK(stat_int(cs, "registry-slots") > 0,
              "registry-slots > 0");
    }
    ...
    // AC2: primitive:generate-skeleton returns AI-friendly bundle
    {
        ...
        CHECK(sk && aura::compiler::types::is_hash(*sk),
              "generate-skeleton returns hash");
        const auto sk_after = stat_int(cs, "skeleton-generations");
        CHECK(sk_after > sk_before,
              std::format("skeleton-generations grew ({} -> {})",
                          sk_before, sk_after));
        const auto cat = hash_str(cs, "(primitive:generate-skeleton \"coverpoint\")",
                                  "category");
        CHECK(cat == "sva",
              std::format("coverpoint description -> sva category (got {})", cat));
        const auto spec = hash_str(cs,
            "(primitive:generate-skeleton \"add coverpoint to covergroup\")",
            "spec");
        CHECK(!spec.empty() && spec.find("coverpoint") != std::string::npos,
              "spec mentions coverpoint");
        const auto cpp = hash_str(cs,
            "(primitive:generate-skeleton \"add coverpoint to covergroup\")",
            "cpp-lambda");
    ...
    // AC3: EDA primitives have category/schema meta via primitive:describe
    {
        ...
        auto cat = cs.eval(
            "(car (cdr (cdr (cdr (cdr (primitive:describe "
            "\"eda:weaken-property\"))))))");
        CHECK(cat && aura::compiler::types::is_string(*cat),
              "category field is string");
        ...
        auto schema = cs.eval(
            "(cdr (cdr (cdr (cdr (cdr (primitive:describe "
            "\"eda:add-coverpoint-bin\"))))))");
        CHECK(schema && aura::compiler::types::is_string(*schema),
              "schema field is string");
    }
    ...
    // AC4: generated skeleton pattern used in verification closed-loop
    {
        ...
        const auto snippet = hash_str(cs,
            "(primitive:generate-skeleton \"verification feedback coverage\")",
            "test-snippet");
        CHECK(!snippet.empty() && snippet.find("eda:run-verification-feedback") !=
                                    std::string::npos,
              "verification skeleton test-snippet references feedback primitive");
        auto r = cs.eval(std::format(
            "(eda:run-verification-feedback \"coverage.log\" \"{} stress\")",
    ...
    // AC5: stats:count
    {
        ...
        CHECK(count && aura::compiler::types::is_int(*count) &&
                  aura::compiler::types::as_int(*count) == 77,
              "stats:count == 77");
    }
    ...
    // AC6: fiber stress — generate-skeleton + describe + EDA mutate
    {
        ...
        auto worker = [&] {
            for (int i = 0; i < k_iters; ++i) {
                ...
                auto r = cs.eval(
                    "(eda:run-verification-feedback \"coverage.log\" \"0 fiber\")");
