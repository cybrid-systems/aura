...
static void run_matrix(CompilerService& cs) {
    ...
    (void)cs.eval(
        "(mutate:rebind \"g\" \"(lambda (y) (+ y 25))\" \"issue-467-dirty\")");
    const auto snap4b = cs.snapshot();
    std::println("  narrowing_dirty_recovery: {} -> {}",
                 snap4a.narrowing_dirty_recovery_total,
                 snap4b.narrowing_dirty_recovery_total);
    CHECK(snap4b.narrowing_dirty_recovery_total >=
              snap4a.narrowing_dirty_recovery_total,
    ...
    std::println("  blame_attached={} blame_complete={} stale_caught={}",
                 snap6b.narrow_blame_attached_total,
    ...
    for (int round = 0; round < 3; ++round) {
        const std::string f_body =
            "(lambda (x) (if (number? x) (+ x " + std::to_string(round + 13) +
            ") 0))";
