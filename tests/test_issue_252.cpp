...
bool test_ffi_closure_bumps() {
    ...
    auto r = cs.eval(std::string("(set-code \"(define g (lambda (x) (+ x 1))) (g 5)\")"));
    if (!r) {
        std::println("  FAIL: set-code failed");
        ++g_failed;
        return false;
...
bool test_bridge_calls_stay_zero() {
    std::println("\n--- AC4: bridge-calls stays 0 when no bridge is set ---");
    aura::compiler::CompilerService cs;
    auto r = cs.eval(std::string("(set-code \"(define g (lambda (y) (+ y 1))) (g 5)\")"));
    if (!r) {
        std::println("  FAIL: set-code failed");
        ++g_failed;
        return false;
...
bool test_closure_stats_primitive() {
    ...
    aura::compiler::CompilerService cs;
    auto r1 = cs.eval(std::string("(set-code \"") + "(define h (closure:stats))" + "\")");
    if (!r1) {
...
bool test_bridge_fraction_observable() {
    ...
    for (int i = 0; i < 3; ++i) {
        auto r = cs.eval(std::string("(set-code \"(define f (lambda (x) (* x 2))) (f ") +
                         std::to_string(i + 1) + ")\")");
        if (!r) {
            std::println("  FAIL: set-code failed");
            ++g_failed;
...
int aura_issue_252_run() {
    return aura_issue_252_detail::run_tests();
}
