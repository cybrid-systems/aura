...
tests = [
    ...
    (
        ...
        "(define (f x) (+ x 1.5)) (display (f 3))",
        "4.5",
        "",
    ),
    (
        "issue-19-coerce-float-int-fn",
        "(define (g x) (+ x 1)) (display (g 3.14))",
        "4.14",
        "",
    ),
    ("issue-19-coerce-int-string-annot", '(: x Int "42")', "42", ""),
    (
        "issue-20-occurrence-basic",
        '(let ((x "hello")) (if (string? x) (string-length x) 0))',
        "5",
        "",
    ),
    (
        "issue-20-occurrence-not",
        '(let ((x "hello")) (if (not (string? x)) 0 (string-length x)))',
        "5",
        "",
    ),
    (
        "issue-20-occurrence-and",
        "(let ((x 42)) (if (and (number? x) (integer? x)) x 0))",
        "42",
        "",
    ),
    (
        "issue-20-occurrence-and-diff",
        '(let ((x "hello")) (if (and (string? x) (number? x)) x "fallback"))',
        "fallback",
        "",
    ),
    (
        "fiber-join-basic",
        "(begin (define fid (fiber:spawn (lambda () (+ 1 2)))) (display (fiber:join fid)))",
        "3",
        "",
    ),
    (
        "fiber-spawn-multi",
        "(begin (define a (fiber:spawn (lambda () (* 3 4)))) (define b (fiber:spawn (lambda () (+ 10 20)))) (display (+ (fiber:join a) (fiber:join b))))",
        "42",
        "",
    ),
    (
        "fiber-stdin-concurrent",
        "(begin (define f1 (fiber:spawn (lambda () (+ 1 2)))) (define f2 (fiber:spawn (lambda () (+ 10 20)))) (define f3 (fiber:spawn (lambda () (+ 100 200)))) (display (+ (fiber:join f1) (fiber:join f2) (fiber:join f3))))",
        "333",
        "",
    ),
    (
        "thread-pool-enqueue-stdin",
        "(display (thread_pool:enqueue (lambda () (+ 1 2))))",
        "3",
        "",
    ),
    ("eval-async-stdin-fallback", '(display (eval:async "(+ 1 2)"))', "3", ""),
    (
        "typecheck-status-ok",
        '(set-code "(define (f x) (+ x 1))") (mutate:rebind "f" "(lambda (x) (+ x 2))") (display (typecheck-status))',
        "ok",
        "",
    ),
    (
        "typecheck-status-after-bad-mutate",
        '(set-code "(define (f x) (+ x 1))") (mutate:rebind "f" "(lambda (x) (undefined-fn x))") (display (typecheck-status))',
        "typecheck after mutate:rebind failed",
        "",
    ),
    (
        "typecheck-status-set-body-ok",
        '(set-code "(define (f x) (+ x 1))") (mutate:set-body "f" "(+ x 2)") (display (typecheck-status))',
        "ok",
        "",
    ),
    ("issue-18-any-annot-let", "(let ((x (: y Any 42))) x)", "42", ""),
    (
        "issue-18-any-annot-fn",
        "(let ((f (lambda (x) (: y Any (+ x 1))))) (f 41))",
        "42",
        "",
    ),
    ("issue-18-any-annot-chain", "(: z Any (+ 1 (: x Int 2)))", "3", ""),
    (
        "issue-18-any-let-poly",
        "(let ((id (lambda (x) x))) (let ((y (: z Any (id 42)))) y))",
        "42",
        "",
    ),
    ...
    (
        "match-missing",
        "(define-type (C) (A) (B) (C)) (let ((x A)) (match x ((A) 1) ((B) 2)))",
        "1",
        "match warning",
    ),
    (
        "match-wildcard",
        "(define-type (C) (A) (B) (C)) (let ((x A)) (match x ((A) 1) ((_) 2)))",
        "1",
        "",
    ),
    (
    ...
    (
        ...
        "(define-type (List a) (Nil) (Cons a (List a)))(let ((xs (Cons 1 Nil))) (match xs ((Cons h t) h)))",
        "1",
        "unhandled",
    ),
    ...
    (
        ...
        "(define-type (List a) (Nil) (Cons a (List a)))(let ((xs Nil)) (match xs (Nil 0)))",
        "0",
        "unhandled",
    ),
    ...
    ("adt-non-adt-subject", '(let ((n 1)) (match n (1 "one") (_ "other")))', "one", ""),
    ...
    ("tc-strict-pass-int-eq-int", "(+ 1 2)", "3", ""),
    ...
    ("file-prim-exists-no", '(file-exists? "/tmp/aura-fp-nope-xyz")', "0", ""),
    (
        "file-prim-delete",
    ...
    ("file-prim-size", '(file-size "/tmp/aura-fp1.txt")', "2", ""),
    (
    ...
    # ── 类型系统 ──────────────────────────────────────────
    ("lambda-annot-param", "((lambda ((: x Int)) (+ x 1)) 41)", "42", ""),
    ("lambda-annot-param2", "((lambda ((: x Int)) x) 99)", "99", ""),
    ("occ-integer", "(let ((x 42)) (if (integer? x) (+ x 1) 0))", "43", ""),
    ("occ-hash", "(let ((x (hash))) (if (hash? x) 42 0))", "42", ""),
    ("let-poly", '((lambda (x) (list (x 1) (x "hi"))) (lambda (y) y))', '(1 "hi")', ""),
    (
        "let-poly-id",
        '(let ((id (lambda (x) x))) (display (id 42))(display (id "hi"))(display (id #t)))',
        "42hi#t",
        "",
    ),
    (
        "let-poly-nested",
        "(let ((f (lambda (x) x)))(let ((g (lambda (y) (f y))))(display (g 42))(display (g #t))))",
        "42#t",
        "",
    ),
    (
        "let-poly-curry",
        '(let ((const (lambda (x) (lambda (y) x))))(display ((const 42) "ignored")))',
        "42",
        "",
    ),
    (
        "let-poly-fst",
        "(let ((fst (lambda (p) (car p))))(display (+ (fst (cons 1 2)) (fst (cons 3 4)))))",
        "4",
        "",
    ),
    (
        "let-poly-multi",
        "(let ((f (lambda (x) x))(g (lambda (x) x)))(display (f 1))(display (g 2)))",
        "12",
        "",
    ),
    (
        "letrec-poly",
        "(letrec ((id (lambda (x) x)))(display (id 99))(display (id #f)))",
        "99#f",
        "",
    ),
    # ── 增量类型检查 #4 ──────────────────────────────────
    (
        "incr-basic",
        '(begin (set-code "(define (add x y) (+ x y))")(display (typecheck-current))(typecheck-current))',
        "type: Void\nno errors",
        "",
    ),
    (
    ...
    ("closure-warning-stdout", "(define (f x) (+ x 1)) f", "uncalled function", ""),
    # ── FFI 诊断 ──────────────────────────────────────────
    (
        "ffi-error-stdout",
    ...
    ("ffi-sig-missing-arrow", '(c-func -1 "strlen" "invalid")', "missing '->'", ""),
    (
        "ffi-sig-unknown-type",
        '(c-func -1 "strlen" "(String) -> Unknown")',
    ...
    (
        ...
        '(c-func -1 "nonexistent_fn_xyz" "(String) -> Int")',
    ...
    ("file-exists", '(file-exists? "/tmp/aura-regr.txt")', "1", ""),
    ("file-exists-not", '(file-exists? "/tmp/aura-nonexistent-xyz")', "0", ""),
    ("file-size", '(file-size "/tmp/aura-regr.txt")', "8", ""),
    (
        "file-delete",
    ...
    ("directory-list", '(directory-list "/tmp")', "aura-regr.txt", ""),
    # ── try-catch ─────────────────────────────────────────
    ("try-catch-ok", '(try "ok" (catch (e) "error"))', "ok", ""),
    ("try-catch-error", '(try (display (car ())) (catch (e) "caught"))', "caught", ""),
    # ── shell / command-output ────────────────────────────
    ("command-output", '(command-output "echo 42")', "42", ""),
    (
        "command-output-multi",
        '(command-output "echo hello; echo world")',
        "hello\nworld",
        "",
    ),
    ("command-output-empty", '(command-output "echo -n")', "", ""),
    # ── command-line ──────────────────────────────────────
    ("cmdline-empty", "(command-line)", "", ""),
    # #32: vector-math
    ("vec-sum", '(require "std/vector-math" all:)(vec:sum (vec:range 0 5))', "10", ""),
    (
        "vec-dot",
