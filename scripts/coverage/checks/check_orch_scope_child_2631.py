...
MVPSCOPE = ROOT / "scripts/coverage/checks/check_orch_mvp_scope.py"
...
def main() -> int:
    ...
    # AC3: scripts/coverage/checks/check_orch_mvp_scope.py still green
    if not MVPSCOPE.exists():
        failures.append("AC3: scripts/coverage/checks/check_orch_mvp_scope.py not found")
    else:
        mvp = MVPSCOPE.read_text(encoding="utf-8", errors="replace")
        if "AgentRegistry" not in mvp or "global_agent_registry" not in mvp:
            failures.append(
                "AC3: scripts/coverage/checks/check_orch_mvp_scope.py must still reject "
