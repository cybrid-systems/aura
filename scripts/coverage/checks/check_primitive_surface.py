...
Usage:
  python3 scripts/coverage/checks/check_primitive_surface.py              # check (CI / gate)
  python3 scripts/coverage/checks/check_primitive_surface.py --strict     # freeze + budget + facade-only
  python3 scripts/coverage/checks/check_primitive_surface.py --write      # refresh inventory
  python3 scripts/coverage/checks/check_primitive_surface.py --update-baseline  # intentional growth
...
INTERIM_HARD_CEILING = 544  # measured on the formatted #3797-#3833 tree; #4101-#4103 wave +1 (full-gate scan, #1448 ceiling procedure); #3969 wave +1 (full-gate scan, #1448 ceiling procedure); #3967/#3968 wave +1 (full-gate scan, #1448 ceiling procedure); #3966 wave +1 (full-gate scan, #1448 ceiling procedure); #3964 wave +1 (full-gate scan, #1448 procedure); #3946-#3957 wave +1 (full-gate scan, #1448 ceiling procedure); #3854-#3856 waves +1 (full-gate scan)  # #3797-#3833 waves: measured gate full-flag scan  # #3797-#3833 waves: query prims per domain (+3 total across 3 waves)  # #3797-#3814 waves: +5 public query prims
...
COMMERCIAL_DOMAIN_BUDGETS: dict[str, int] = {
    "git-": 39,  # #1970 — git integration; AURA_ENABLE_GIT (≠ AURA_HAVE_LIBGIT2); #3615 ship raise (was 14; #3461 refresh + mergebot landings); #3850-#3853 waves +1; #3854-#3856 waves +1; #3867 wave: closures shard conversion (full-gate measured +1/domain twice, #1967 raise procedure); #4101-#4103 concurrent wave: full-gate measured +1/domain (#1967 raise procedure); #4094-#4104 concurrent ship wave: full-gate measured 38/34/36/36/41 +2 margin (raise procedure)
    "strategy:": 37,  # #1973 — evolution controller; AURA_ENABLE_STRATEGY; #3615 ship raise (was 11; #3461 refresh + mergebot landings); #3850-#3853 waves +1; #3854-#3856 waves +1; #3867 wave: closures shard conversion (full-gate measured +1/domain twice, #1967 raise procedure); #3963 sharded-lock wave: full-gate measured +1 (#1967 raise procedure); #4101-#4103 concurrent wave: full-gate measured +1/domain (#1967 raise procedure); #4094-#4104 concurrent ship wave: full-gate measured 38/34/36/36/41 +2 margin (raise procedure)
    "synthesize:": 37,  # #1974 — synthesis templates/LLM/GA; AURA_ENABLE_SYNTHESIZE; #3615 ship raise (was 11; #3461 refresh + mergebot landings); #3850-#3853 waves +1; #3854-#3856 waves +1; #3867 wave: closures shard conversion (full-gate measured +1/domain twice, #1967 raise procedure); #3963 sharded-lock wave: full-gate measured +1 (#1967 raise procedure); #4101-#4103 concurrent wave: full-gate measured +1/domain (#1967 raise procedure); #4094-#4104 concurrent ship wave: full-gate measured 38/34/36/36/41 +2 margin (raise procedure)
    "tcp-": 42,  # #1975 client (4) + #2771 server listen/accept/timeout/local-port (4) + #3379/#3380 mergebot land; #3615 ship raise (was 15; #3461 refresh + mergebot landings — unit-test scan 17 not 16); #3850-#3853 waves +1; #3854-#3856 waves +1; #3867 wave: closures shard conversion (full-gate measured +1/domain twice, #1967 raise procedure); #3963 sharded-lock wave: full-gate measured +1 (#1967 raise procedure); #4101-#4103 concurrent wave: full-gate measured +1/domain (#1967 raise procedure); #4094-#4104 concurrent ship wave: full-gate measured 38/34/36/36/41 +2 margin (raise procedure)
    "m4-": 35,  # #1976 — M4 linear stubs (move/borrow/return!); AURA_ENABLE_M4; #3615 ship raise (was 10; #3461 refresh + mergebot landings); #3850-#3853 waves +1; #3854-#3856 waves +1; full-gate measured +1/domain again (#1967 raise procedure); #3867 wave: closures shard conversion (full-gate measured +1/domain twice, #1967 raise procedure); #4101-#4103 concurrent wave: full-gate measured +1/domain (#1967 raise procedure); #4094-#4104 concurrent ship wave: full-gate measured 38/34/36/36/41 +2 margin (raise procedure)
}
    ...
    for p in sorted(COMMERCIAL_DOMAIN_BUDGETS.keys()):
        ...
        if n > budget:
            print(
                ...
                f"COMMERCIAL_DOMAIN_BUDGETS in scripts/coverage/checks/check_primitive_surface.py "
...
def main() -> int:
    ...
    if new:
        ...
        print(
            ...
            "  python3 scripts/coverage/checks/check_primitive_surface.py --update-baseline\n"
