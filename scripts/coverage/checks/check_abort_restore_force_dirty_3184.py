...
def main() -> int:
    ...
    # Issue #3897: every abort_restore_dual_topology call site passes the
    # enter dirty snapshot (persist-reject, inbody, force-rollback, Strict
    # reflect-validate, and the synth-hard-fail success arm).
    n_call = mb3865.count("workspace_flat_->abort_restore_dual_topology(")
    n_move = mb3865.count("std::move(cp.dirty_soa_snapshot)")
    if n_call != 5:
        fails.append(f"AC3897: expected 5 abort_restore_dual_topology call sites, got {n_call}")
    if n_move != 5:
        fails.append(f"AC3897: expected 5 std::move(cp.dirty_soa_snapshot), got {n_move}")
