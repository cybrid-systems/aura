...
def render_md(rows: list[RootTest]) -> str:
    ...
    lines.append("")
    lines.append("## Wave 0 — safe streamline (no coverage loss)")
    lines.append("")
    lines.append("**Status: applied** (classification tooling + stub cleanup).")
    lines.append("")
    lines.append("1. ~~Delete empty Phase-2 stubs~~ → removed `test_fiber` / `test_mutation` /")
    lines.append("   `test_observability` / `test_persist` (coverage in `tests/domain/*`)")
    lines.append("2. ~~EXCLUDE_FROM_ALL~~ `test_open_issues_phase1_batch` (alias of `test_obs_schema_matrix`;")
    lines.append("   kept for `jit_late3` bundle membership only)")
    lines.append("3. Prefer **extend domain/** over new root binaries (see tests/README.md)")
    lines.append("")
    ...
    for theme in THEME_ORDER:
        ...
        lines.append("")
    lines.append("## Streamline roadmap")
    lines.append("")
    lines.append("| Wave | Focus | Expected reduction |")
    lines.append("|------|-------|-------------------:|")
    lines.append("| 0 | Empty stubs + obs alias EXCLUDE | 4–5 binaries off default path |")
    lines.append("| 1 | Thin schema probes → `obs_schema_cases.hpp` | 20–40 root targets |")
    lines.append("| 2 | Production family → one domain matrix + keep hard mixed | 20–26 EXCLUDE |")
    lines.append("| 3 | Near-dup task pairs merge into domain theme suites | 10–20 files |")
    lines.append("| 4 | Promote root `test_issue_19xx` ACs into domain/ | 8 renames |")
    lines.append("")
