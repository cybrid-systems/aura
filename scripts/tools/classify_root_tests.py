...
Complements scripts/tools/inventory_legacy_tests.py (#1957, issues/ + theme inventory).
...
Usage:
  python3 scripts/tools/classify_root_tests.py              # write report
  python3 scripts/tools/classify_root_tests.py --check      # fail if report stale
  python3 scripts/tools/classify_root_tests.py --json out.json
...
def render_md(rows: list[RootTest]) -> str:
    ...
    lines.append("")
    lines.append(f"**Generated:** {date.today().isoformat()} by `scripts/tools/classify_root_tests.py`")
    ...
    if args.check:
        ...
        if not m or m.group(1) != h:
            print(
                ...
                f"Re-run: python3 scripts/tools/classify_root_tests.py",
                file=sys.stderr,
            )
            return 1
