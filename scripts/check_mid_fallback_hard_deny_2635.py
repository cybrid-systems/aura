...
def main() -> int:
    ...
    linter_2493 = _read("scripts/check_audit_mutation_id_unify_2493.py")
    linter_2594 = _read("scripts/check_audit_mid_fallback_slo_2594.py")
    ...
    for linter in ("check_audit_mutation_id_unify_2493.py", "check_audit_mid_fallback_slo_2594.py"):
        r = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / linter)],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
