# 0 "/home/dev/code/aura/src/compiler/evaluator.ixx"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3
# 0 "<command-line>" 2
# 1 "/home/dev/code/aura/src/compiler/evaluator.ixx"
...
private:
    ...
    void* type_registry_ = nullptr;
    std::unordered_map<ClosureId,Closure> closures_;
    ClosureBridgeFn closure_bridge_;
    ModuleLoadedFn module_loaded_cb_;
    std::unordered_map<std::string, MacroDef> macros_;
    std::vector<Env*> modules_;
    std::unordered_map<std::string, std::uint64_t> module_cache_;
    std::unordered_set<std::string> loading_stack_;
    std::vector<std::string> module_names_;
    std::vector<types::EvalValue> cells_;
    std::vector<Pair> pairs_;
    std::vector<types::EvalValue> error_values_;
