...
| 优先级 | 项目 | 工作量 | 描述 |
|--------|------|--------|------|
| **P0** | I/O (文件读写) | 🔜 1d | `read-file`, `write-file`, `file-exists?` — Agent 写出真实程序 |
| **P0** | cond / and / or | ⬜ 0.5d | 条件语法糖，替代嵌套 if |
| **P1** | 卫生宏 (hygienic) | ⬜ 3d | 自动重命名 + FlatAST 标记 + 预展开 |
| **P1** | 向量/哈希表 | ⬜ 2d | 真实程序需要的数据结构 |
| **P2** | CI 基准回归 | ⬜ 0.5d | benchmark --check 自动化 |
| **P2** | serve 协议完善 | ⬜ 1d | 文件操作命令 |
| **P3** | L6.8+ 多态类型 | ⬜ 5d | ∀a. T 多态 + 类型类 |
| **P3** | M4 LLVM + AOT | ⬜ 长期 | LLVM 后端 + 原生编译 |
