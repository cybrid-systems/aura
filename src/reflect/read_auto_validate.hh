...
template <typename T> consteval const char* validate_node() {
    ...
    for (std::size_t i = 1; i < members.size(); ++i) {
        ...
        bool ok = d == "long int" || d == "unsigned long" || d == "int" || d == "unsigned int" ||
                  d == "short" || d == "unsigned short" || d.starts_with("std::string") ||
                  d.find("std::__cxx11::basic_string") != std::string_view::npos ||
                  d.starts_with("Expr*") || d.find("vector<") != std::string_view::npos;
...
template <typename T> consteval void check() {
