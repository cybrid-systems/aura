...
consteval std::string_view type_name_for_schema(std::meta::info type) {
    ...
    if (is_same_type(type, ^^int8_t))
        return "integer";
    if (is_same_type(type, ^^uint8_t))
        return "integer";
    if (is_same_type(type, ^^char))
        return "integer";
    if (is_same_type(type, ^^signed char))
        return "integer";
    ...
    if (is_same_type(type, ^^int))
        return "integer";
    if (is_same_type(type, ^^unsigned))
        return "integer";
    if (is_same_type(type, ^^unsigned int))
        return "integer";
    if (is_same_type(type, ^^long))
        return "integer";
    if (is_same_type(type, ^^unsigned long))
        return "integer";
    if (is_same_type(type, ^^long long))
        return "integer";
    if (is_same_type(type, ^^unsigned long long))
        return "integer";
...
template <typename T> consteval std::size_t schema_buffer_size() {
...
template <typename T> struct schema_storage {
...
template <typename T> std::string_view get_json_schema() {
