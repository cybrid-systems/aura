// lifetime_pin.hh — SSOT for LifetimePin (Phase 3 sharded registry).
//
// Architecture: single implementation for both `#include` and
// `import aura.core.lifetime_pin` consumers. The module unit
// (lifetime_pin.ixx) only re-exports this header (resource_quota pattern).
// Dual Phase-2 header / Phase-3 module drift is intentionally eliminated.
//
// Do NOT reintroduce a second LifetimePin / pin registry in the .ixx.
// TUs must not both `#include` and `import` this API in the same TU
// (ambiguous overload under GCC modules). Prefer:
//   Module consumers: import aura.core.lifetime_pin;
//   Non-module TUs (JIT runtime, security_defaults): #include this header.
//
// Issue #2678: SSOT + dual-include ban (was: bulk ops module-only).
// See lifetime_pin.ixx banner for Phase history (#2000/#2048/#2265/#2342/…).
...
public:
    ...
    // stamp fields. Matches the existing 2-arg validate() semantics.
