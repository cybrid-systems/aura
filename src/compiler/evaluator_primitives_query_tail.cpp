...
void register_query_tail_primitives(PrimRegistrar add, std::pmr::vector<Pair>& pairs,
    ...
    // Issue #3736: query:aot-incremental-reemit-stats is registered once
    // in register_eval_observability_primitives (obs_eval last-wins).
    // Dirty-region counts live as additive keys on that hash
    // (dirty-region-reemit-total / -success-total). Do not re-register
    ...
    add("query:schema", [&string_heap, &type_registry](std::span<const EvalValue> a) -> EvalValue {
        ...
        // Issue #3906: query:macro-provenance-stats is registered only in
        // evaluator_primitives_obs_jit.cpp (hash + schema-2235 / cross-flat keys).
        // This nested int-sum duplicate (#1908) last-write-won; catalog seed
        // (#3603) is unchanged.
