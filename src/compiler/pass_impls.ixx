// Issue #2524 Phase C: concrete pass implementations extracted from
// pass_manager.ixx. Importers should continue to `import aura.compiler.pass_manager`
// (facade re-exports this module). Hot TUs that only need pipeline folds can
// import aura.compiler.pass_pipeline_core instead.
module;
#include <algorithm>
#include <array>
#include <atomic>
...
// Layering: Compiler → Core (Issue #1885).
...
public:
    ...
    // define-level mask is wired via set_block_dirty_pred / define_cache.
    ...
    bool run_columnar_block(IRFunctionSoA& func, BasicBlockSoA& block) {
        ...
        do {
            ...
            for (std::uint32_t i = start; i < end; ++i) {
                ...
                if (instruction_dirty_pred_ && !is_instruction_dirty(block.block_id, local_i)) {
                    instr_level_pass_skipped_clean_total.fetch_add(1, std::memory_order_relaxed);
                    continue;
                }
...
private:
    ...
    void run_on_function(aura::ir::IRFunction& func) {
        ...
        const bool dirty_mode = static_cast<bool>(block_dirty_pred_);
...
private:
    ...
    std::uint64_t pipeline_epoch_ = 0;
    BlockDirtyPred block_dirty_pred_; // Issue #1574
};
...
public:
    ...
    // instruction_dirty_pred_ is set (ImpactScope precision).
    void run_on_function(aura::ir::IRFunction& func,
