...
std::string GitContext::status_short() const {
    ...
    if (git_status_list_new(&status, repo_, nullptr) != 0) return "";
    std::string out;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        char ix = '?', ix2 = ' ';
        if      (e->status & GIT_STATUS_INDEX_NEW)      ix = 'A';
        else if (e->status & GIT_STATUS_INDEX_MODIFIED) ix = 'M';
        else if (e->status & GIT_STATUS_INDEX_DELETED)  ix = 'D';
        else if (e->status & GIT_STATUS_INDEX_RENAMED)  ix = 'R';
        else if (e->status & GIT_STATUS_INDEX_TYPECHANGE) ix = 'T';

        char wt = '?', wt2 = ' ';
        if      (e->status & GIT_STATUS_WT_NEW)      wt = '?';
        else if (e->status & GIT_STATUS_WT_MODIFIED)  wt = 'M';
        else if (e->status & GIT_STATUS_WT_DELETED)   wt = 'D';
        else if (e->status & GIT_STATUS_WT_RENAMED)   wt = 'R';
        else if (e->status & GIT_STATUS_WT_TYPECHANGE) wt = 'T';
        else if (e->status & GIT_STATUS_WT_UNREADABLE) wt = '!';

        const char* path = e->head_to_index
...
std::string GitContext::log_oneline(int n) const {
    ...
    if (git_revwalk_new(&walker, repo_) != 0) return "";
    ...
    while (count < n && git_revwalk_next(&oid, walker) == 0) {
        ...
        if (git_commit_lookup(&commit, repo_, &oid) == 0) {
            ...
            if (nl != std::string::npos) subject = subject.substr(0, nl);
...
std::string GitContext::branch_current() const {
    ...
    if (git_repository_head(&head, repo_) != 0) return "";
...
std::string GitContext::rev_parse_short() const {
    if (!repo_) return "";
    git_object* head_obj = nullptr;
    if (git_revparse_single(&head_obj, repo_, "HEAD") != 0) return "";
...
int GitContext::commit(const std::string& message) {
    ...
    do {
        if (git_repository_index(&index, repo_) != 0) { err = -1; break; }
        if (git_index_write_tree(&tree_oid, index) != 0) { err = -1; break; }
        if (git_tree_lookup(&tree, repo_, &tree_oid) != 0) { err = -1; break; }
        ...
        if (git_signature_default(&sig, repo_) != 0) { err = -1; break; }

        const git_commit* parents[1] = {parent};
        err = git_commit_create(
            &commit_oid, repo_, "HEAD", sig, sig,
            "UTF-8", message.c_str(),
...
int GitContext::stage(const std::vector<std::string>& paths) {
    ...
    do {
        ...
        for (auto& p : path_storage) path_ptrs.push_back(p.data());
...
std::string GitContext::status_short() const { return ""; }
std::string GitContext::diff(bool) const { return ""; }
std::string GitContext::log_oneline(int) const { return ""; }
std::string GitContext::branch_current() const { return ""; }
std::string GitContext::rev_parse_short() const { return ""; }
int GitContext::commit(const std::string&) { return -1; }
int GitContext::stage(const std::vector<std::string>&) { return -1; }
