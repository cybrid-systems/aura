    ...
    ObservabilityPrims::register_stats_impl(
        ...
        [&ev, metrics](std::span<const EvalValue>) -> EvalValue {
            ...
            std::vector<std::pair<std::string, EvalValue>> kv = {
                ...
                // Issue #2110: hybrid string ↔ NodeId DepGraph cascade
                {"dep-graph-node-mirror-edges-total",
                 make_int(m ? load_u64(m, m->dep_graph_node_mirror_edges_total) : 0)},
                {"dep-graph-hybrid-cascade-hits",
                 make_int(m ? load_u64(m, m->dep_graph_hybrid_cascade_hits) : 0)},
                {"dep-graph-hybrid-cascade-wired", make_int(1)},
                {"schema-2110", make_int(2110)},
                {"issue-2110", make_int(2110)},
                // Issue #2187: block-level DepGraph edges (schema-2110 lineage)
                {"dep-graph-block-mirror-edges-total",
                 make_int(m ? load_u64(m, m->dep_graph_block_mirror_edges_total) : 0)},
                {"dep-graph-node-cascade-block-hits",
                 make_int(m ? load_u64(m, m->dep_graph_node_cascade_block_hits) : 0)},
