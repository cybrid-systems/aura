...
// Issue #3028: same-FlatAST reject + steal abort (explicit-depth residual).
...
// Issue #3981: 16-slot name_map pin + nested steal inject (weak steal-total==0).
...
// Issue #3341: stamp Evaluator::last_mutate_error_ = "steal-abort-mid-expand"
// when an Evaluator is resolvable. No-op if none is wired.
...
// Issue #3750: name-precise peer AOT slot stale (not all-slot #3070).
void aura_aot_soft_stale_peer_slots_for_name(const char* name, void* owner);
int aura_aot_peer_jit_name_hash_is_soft_stale(std::uint64_t name_hash);
...
// Issue #3747: production also drains after named+captured remount
// (reload commit / reemit) so query-only hosts do not keep the old
// module mapped. Soft still BoundaryExit-only.
void aura_force_drain_old_so(void);
std::uint64_t aura_reload_old_so_staged_total_v_read(void);
std::uint64_t aura_reload_old_so_pending_v_read(void);
...
// Returns 1 if unsafe (deopt), 0 if safe.
// Issue #3654: Production/Full + no callback / no env context → 1 (deopt).
// Soft/Off + no callback / no context → 0 (pass-through).
// Always bumps jit_linear_post_mutate_enforcements_total when callback set
// or when Production/Full takes the unset-callback fail-closed arm.
