...
struct CompilerSnapshot {
    ...
    // Issue #1474 / #1495: per-block re-lower observability. Mirrors
    ...
    // full_relower_count mirrors relower_full_called_count (#1495 AC3).
    std::uint64_t incremental_relower_blocks_total = 0;
    std::uint64_t full_relower_count = 0;
    ...
    // Issue #1502: parent_ topology restore / rebuild count.
    std::uint64_t parent_topology_restore_count = 0;
