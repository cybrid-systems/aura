...
struct CompilerMetrics {
    ...
    // Pair them on dashboards to distinguish "env_gen drift" from
    // "defuse drift" under compact_env_frames + RegionExclusive
    // writers.
    std::atomic<std::uint64_t> closure_capture_env_gen_mismatch_total{0}; // #2272
    ...
    // Issue #2254 / #2520: SoA single source of truth observability.
    //   - soa_only_path_total: # of lowers under AURA_IR_SOA_ONLY=1
    //     (production default; the SoA emit path bumps this)
    //   - residual_aos_bridge_total: # of residual SoA→AoS materializes
    //     (to_aos_view / dual-emit). Issue #2520: TEST-ONLY metric under
    //     production SoA-only — target 0 on production smoke; non-zero
    //     without AURA_ALLOW_AOS_BRIDGE hard-fails at the call site.
    std::atomic<std::uint64_t> soa_only_path_total{0};
    std::atomic<std::uint64_t> residual_aos_bridge_total{0}; // #2520 test-only on prod
    ...
    // aura_reload_aot_module_for_eval); `aot_reload_fall_back_jit_only_total`
    // counts exhausted windows where the policy directed the host
    // to fall back to JIT (Version/Defuse/Env/Linear with
    // fall_back_jit_only=true). Reset-for-test helper
    // (reset_observability_metrics_for_test, if added) clears
    ...
    // Issue #2088: unified GcDeferReason bitmask per-reason arm counts +
    ...
    // ── Issue #1952 / #1930: LLVM re-emit success + stable func_id map ──
    // aot_incremental_reemit_success_total: bumped when host-wired
    //   aura_set_aot_emit_fn returns true. Pairs with
    //   aot_incremental_reemit_count (delta ≈ failures when emit wired).
    // stable_func_id_preserved_total: bumped when name already in the
    //   process-stable name→func_id map (#1930 real map; re-emit reuse).
    // stable_func_id_assigned_total: bumped when a new name first enters
    //   the map (first emit / first skeleton would-reemit of that name).
    ...
    // aot_evolution_region_skips_total: permanent Evolution (region=2) excludes
