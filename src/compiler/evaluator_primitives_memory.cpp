...
void register_memory_primitives(PrimRegistrar add, Evaluator& ev,
    ...
    add("gc-heap", [&ev, destroy_defuse_index](const auto&) -> EvalValue {
        ...
        // multi-lock sites reverse the order.
        {
            ...
            ev.clear_sym_intern(); // Issue #3457: heap indices die with string_heap_
    ...
    // Issue #3906: query:arena-live-compact-stats is registered only in
    // evaluator_primitives_obs_eval.cpp (richer GOP / pin / remap keys).
    // This memory.cpp duplicate (#1518) last-write-won; catalog seed (#3603)
    // is unchanged.
