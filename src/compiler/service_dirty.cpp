...
void CompilerService::invalidate_function(const std::string& name) {
    ...
    // AURA_CROSS_EVAL_HARD_OWNER_SCOPED=0 keep force-bump (#2841 Soft
    // ergonomics). True process-wide recovery (reload fall_back_jit_only)
    // still notes force-bump. Cascade owner TLS stamped in helper.
    if (aura_aot_cross_eval_hard_owner_scoped_armed() != 0) {
        aura_aot_note_cross_eval_hard_owner_scoped(); // Issue #2951
    } else {
        aura_aot_note_cross_eval_epoch_force_bump(); // Issue #2841 Soft/opt-out
    }
    atomic_bump_epochs_and_stamp_bridge(name);
    ...
    auto try_partial_invalidate_relower = [&](const std::string& fname) -> bool {
        ...
        if (!gate_partial_soa_dirty_sync_(vit->second))
            return false;
        // Issue #2206: recover_source_to_ir_map_desync lives in
        // prepare_source_to_ir_map_for_partial_ so workspace and
        // invalidate share one fail-closed map gate (ensure + recover
        // + missing-instr-loc). Recovered maps keep partial; only a
        // failed recover notes MapInconsistent. source_to_ir_desync_recovered_total
        // still advances on the recover path inside prepare.
        // Issue #3068: same prepare before the impact snapshot.
        if (!prepare_source_to_ir_map_for_partial_(vit->second)) {
...
// Issue #3045 / #3358: AST exhaustiveness cone-force lives in
// dirty::force_adt_exhaust_sites_into_cone (infer_flat_partial /
// evaluator_typecheck). #3358 expands the enclosing parent (cone of 1)
// on ReplaceType / constructor mutate. This IR cascade stays
// function/block scoped; match sites enter via encode_ast_dep_node
// marks from that helper.
