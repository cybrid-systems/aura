...
void register_query_type_stats_primitives(PrimRegistrar add, std::pmr::vector<Pair>& pairs,
    ...
    ObservabilityPrims::register_stats_impl("query:type-incremental-fidelity-stats",
                                            [&string_heap](
                                                ...
                                                // Issue #3003: Production fail-closed after
                                                // solve_delta not SOLVED.
                                                {
                                                    using aura::compiler::typed_audit::
                                                        g_typed_mutation_audit_counters;
                                                    insert_kv(
                                                        "delta-timeout-fail-closed-total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_timeout_fail_closed_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "delta-timeout-fail-closed-wired",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_timeout_fail_closed_wired
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv("schema-3003", 3003);
                                                    insert_kv("issue-3003", 3003);
                                                }
                                                // Issue #2900: SolverBudget Agent TIMEOUT policy
                                                // surface. Additive to #2277; production cannot
                                                // disable escalate.
                                                {
                                                    using aura::compiler::typed_audit::
                                                        g_typed_mutation_audit_counters;
                                                    ...
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .solver_budget_full_escalate_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "solver-budget-instance-repair-prefer-"
                                                        "total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .solver_budget_instance_repair_prefer_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv("solver-budget-wired", 1);
                                                    insert_kv("schema-2900", 2900);
                                                    insert_kv("issue-2900", 2900);
                                                    // Issue #2963: production
                                                    // instance-repair-before-full surface. Additive
                                                    // to #2900 / #2277; Soft quiet remains
                                                    // zero-cost.
                                                    insert_kv(
                                                        "delta-instance-repair-total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_instance_repair_total.load(
                                                                    std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "delta-instance-repair-resolved-total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_instance_repair_resolved_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "delta-timeout-full-after-repair-total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_timeout_full_after_repair_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv("delta-instance-repair-wired", 1);
                                                    insert_kv("schema-2963", 2963);
                                                    insert_kv("issue-2963", 2963);
                                                }
                                                // Issue #2913: solve_delta locality SLO (anti
                                                // silent under-constrain). Soft residual → observe;
                                                // production/Full residual → escalate/reject.
                                                // Existing #1871 locality-hits/misses preserved
                                                // above.
                                                {
                                                    using aura::compiler::typed_audit::
                                                        g_typed_mutation_audit_counters;
                                                    const std::int64_t loc_obs = static_cast<
                                                        std::int64_t>(
                                                        g_typed_mutation_audit_counters
                                                            .solve_delta_locality_slo_observe_total
                                                            .load(std::memory_order_relaxed));
                                                    const std::int64_t loc_esc =
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .solve_delta_locality_escalate_total
                                                                .load(std::memory_order_relaxed));
                                                    const std::int64_t loc_rej =
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .solve_delta_locality_reject_total
                                                                .load(std::memory_order_relaxed));
                                                    insert_kv(
                                                        "solve-delta-locality-slo-observe-total",
                                                        loc_obs);
                                                    insert_kv("solve-delta-locality-escalate-total",
                                                              loc_esc);
                                                    insert_kv("solve-delta-locality-reject-total",
                                                              loc_rej);
                                                    ...
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_locality_budget_allow_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "delta-locality-budget-escalate-total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_locality_budget_escalate_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "delta-locality-budget-pending-handoff-"
                                                        "total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                .delta_locality_budget_pending_handoff_total
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv("delta-locality-budget-wired", 1);
                                                    insert_kv("schema-2994", 2994);
                                                    insert_kv("issue-2994", 2994);
                                                    // Issue #3031: pending_full_solve / locality
                                                    // residual at commit.
                                                    insert_kv(
                                                        "pending-full-solve-residual-last",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                pending_full_solve_residual_last_v_read()));
                                                    insert_kv(
                                                    ...
                                                    insert_kv(
                                                        "pending-full-solve-residual-escalate-"
                                                        "total",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                    ...
                                                    insert_kv(
                                                        "pending-full-solve-residual-wired",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                ...
                                                // + reverify roots.
                                                {
                                                    ...
                                                    // Issue #3005: dirty-cone seed + Production no
                                                    // Dynamic slide.
                                                    const std::int64_t cone_seed =
                                                        m ? static_cast<std::int64_t>(
                                                                m->adt_exhaust_cone_seed_total.load(
                                                                    std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t prod_rej =
                                                        m ? static_cast<std::int64_t>(
                                                                m->adt_exhaust_production_reject_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t soft_obs =
                                                        m ? static_cast<std::int64_t>(
                                                                m->adt_exhaust_soft_observe_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t dyn_prev =
                                                        m ? static_cast<std::int64_t>(
                                                                m->adt_exhaust_dynamic_slide_prevented_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    insert_kv("adt-exhaust-cone-seed-total",
                                                              cone_seed);
                                                    insert_kv("adt-exhaust-production-reject-total",
                                                              prod_rej);
                                                    insert_kv("adt-exhaust-soft-observe-total",
                                                              soft_obs);
                                                    insert_kv(
                                                        "adt-exhaust-dynamic-slide-prevented-total",
                                                        dyn_prev);
                                                ...
                                                // rehydrate.
                                                {
                                                    ...
                                                    // Issue #2896: production-default outermost
                                                    // success persist
                                                    // + fence rehydrate face latch (#2704 wired).
                                                    // Additive; #2608/#2641/#2704/#2842 surfaces
                                                    // preserved.
                                                    insert_kv("occurrence-persist-production-"
                                                              "default-wired",
                                                              1);
                                                    insert_kv("occurrence-persist-outermost-"
                                                              "success-wired",
                                                              1);
                                                    insert_kv("schema-2896", 2896);
                                                    insert_kv("issue-2896", 2896);
                                                    // Issue #2910: densify/steal green stamps
                                                    // freeze CS truth after rehydrate (close empty
                                                    // priority roots residual).
                                                    insert_kv("occurrence-persist-stamp-after-"
                                                              "rehydrate-wired",
                                                              1);
                                                    insert_kv("occurrence-persist-production-"
                                                              "always-on-success",
                                                    ...
                                                    // Issue #2981: same-txn proof reject on
                                                    // empty-after-fence miss.
                                                    insert_kv(
                                                        "type-linear-proof-reject-empty-after-"
                                                        "fence-total",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                type_linear_proof_reject_empty_after_fence_total_v_read()));
                                                    insert_kv(
                                                        "type-linear-proof-empty-after-fence-wired",
                                                        1);
                                                    insert_kv("schema-2981", 2981);
                                                    insert_kv("issue-2981", 2981);
                                                    // Issue #2938: outermost success freezes
                                                    // Occurrence into immutable commit snapshot +
                                                    // post-persist proof stamp. Additive;
                                                    // Soft/empty leave counters at 0.
                                                    insert_kv(
                                                        "occurrence-commit-snapshot-written-total",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                    ...
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                occurrence_commit_snapshot_mid_v_read()));
                                                    insert_kv("occurrence-commit-snapshot-wired",
                                                              1);
                                                    insert_kv("schema-2938",
                                                              aura::compiler::typed_audit::
                                                                  kOccurrenceCommitSnapshotIssue);
                                                    insert_kv("issue-2938",
                                                              aura::compiler::typed_audit::
                                                                  kOccurrenceCommitSnapshotIssue);
                                                    // Issue #3004: persist + Full audit atomic with
                                                    // query:type.
                                                    insert_kv(
                                                    ...
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_persist_audit_atomic_wired
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv(
                                                        "schema-3004",
                                                        aura::compiler::typed_audit::
                                                            kOccurrencePersistAuditAtomicIssue);
                                                    insert_kv(
                                                        "issue-3004",
                                                        aura::compiler::typed_audit::
                                                            kOccurrencePersistAuditAtomicIssue);
                                                    ...
                                                    // (additive).
                                                    insert_kv(
                                                        "occurrence-commit-health-faces",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_commit_health_faces
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_commit_health_goals_live
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_commit_health_persist_size
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_commit_health_needs_recover
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv(
                                                    ...
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                g_occurrence_commit_health_fingerprint_ok
                                                                    .load(
                                                                        std::
                                                                            memory_order_relaxed)));
                                                    insert_kv("occurrence-commit-health-wired", 1);
                                                    insert_kv("schema-2995",
                                                              aura::compiler::typed_audit::
                                                                  kOccurrenceCommitHealthIssue);
                                                    insert_kv("issue-2995",
                                                              aura::compiler::typed_audit::
                                                                  kOccurrenceCommitHealthIssue);
                                                    // Issue #3030: abort/restore clears
                                                    // TypeLinearCommitProof face.
                                                    insert_kv(
                                                    ...
                                                    insert_kv(
                                                        "type-linear-proof-cleared-on-abort-"
                                                        "observe-total",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                ...
                                                // unit tests).
                                                {
                                                    ...
                                                    // TypeLinearCommitProof at stamp (#2758
                                                    // residual). live_goal_count from CS size +
                                                    // bounded goal_fingerprint so Agents detect
                                                    // densify/ steal content drift without N-key
                                                    // join. Additive only — #2613/#2697/#2717/#2758
                                                    // preserved.
                                                    {
                                                        insert_kv(
                                                            ...
                                                            "fingerprint",
                                                            static_cast<std::int64_t>(
                                                                last_proof_goal_fingerprint_v_read()));
                                                        insert_kv(
                                                    ...
                                                    // production default records.
                                                    {
                                                        ...
                                                        // #2188 / #2347 counters.
                                                        {
                                                            ...
                                                            // / #2188 surfaces.
                                                            {
                                                                ...
                                                                // Issue #2847: region
                                                                // type/occurrence commit bind —
                                                                // Soft observe / production reject
                                                                // when OccurrenceGoal pred bits
                                                                // fall outside admitted cone mask.
                                                                // Additive — all
                                                                // #2724/#2754/#2757/#2760/#2761
                                                                // above.
                                                                insert_kv(
                                                                    "region-type-cross-talk-"
                                                                    "observe-total",
                                                                    static_cast<std::int64_t>(
                                                                        aura::compiler::typed_audit::
                                                                            region_type_cross_talk_observe_total_v_read()));
                                                                insert_kv(
                                                                    "region-type-cross-talk-reject-"
                                                                    "total",
                                                                    static_cast<std::int64_t>(
                                                                        aura::compiler::typed_audit::
                                                                            region_type_cross_talk_reject_total_v_read()));
                                                                insert_kv(
                                                                    "region-type-cross-talk-wired",
                                                                    static_cast<std::int64_t>(
                                                                        aura::compiler::typed_audit::
                                                                ...
                                                                // Issue #2932: hold-budget overtime
                                                                // forced outermost fail-closed
                                                                // (safepoint edge, not cooperative
                                                                // Phase-5-only). Additive — all
                                                                // #2701/#2720/#2726 surfaces
                                                                // preserved.
                                                                insert_kv(
                                                                    "mutation-hold-budget-forced-"
                                                                    "fail-"
                                                                    "closed-total",
                                                                    static_cast<std::int64_t>(
                                                                ...
                                                                // Issue #2999: outermost dtor
                                                                // consume (exit half of #2932).
                                                                // Additive — in-body window still
                                                                // requires force-safepoint to enter
                                                                // dtor.
                                                                insert_kv(
                                                                    "mutation-hold-budget-forced-"
                                                                    "fail-"
                                                                ...
                                                                // Issue #3035: P0 force-unlock +
                                                                // dual-topology restore on
                                                                // hold-budget cancel for a
                                                                // non-yield body. #2932/#2999
                                                                // fail-closed at safepoint/dtor
                                                                // edges, but a body that never
                                                                // polls the flag can keep
                                                                // workspace_mtx_ + fiber depth slot
                                                                // held indefinitely. #3035 forces
                                                                // the boundary fail-closed on
                                                                // outermost dtor cancel consume
                                                                // (success=false even without a
                                                                // success flag) so
                                                                // exit_mutation_boundary runs
                                                                // abort_restore_dual_topology +
                                                                // dual canary and Phase-5
                                                                // force-releases the lock + clears
                                                                // the depth slot. Additive — all
                                                                // #2932/#2999 surfaces preserved.
                                                                insert_kv(
                                                                    "mutation-hold-budget-forced-"
                                                                    "unlock-total",
                                                                    static_cast<std::int64_t>(
                                                                    ...
                                                                    // / #2672 surfaces.
                                                                    {
                                                                        ...
                                                                        // / #2622 / #2672 surfaces.
                                                                        {
                                                                            ...
                                                                            insert_kv(
                                                                                ...
                                                                                "wired",
                                                                                1);
                                                                            insert_kv("schema-2981",
                                                                                      2981);
                                                                            insert_kv("issue-2981",
                                                                                      2981);
                                                                        }
                                                ...
                                                // side effects.
                                                {
                                                    ...
                                                    using aura::compiler::typed_audit::
                                                        refined_consistency_drift_face_hit;
                                                    using aura::compiler::typed_audit::
                                                        refined_consistency_observe_total_v_read;
                                                    using aura::compiler::typed_audit::
                                                        refined_consistency_recover_total_v_read;
                                                    using aura::compiler::typed_audit::
                                                        refined_consistency_reject_total_v_read;
                                                    using aura::compiler::typed_audit::
                                                        refined_consistency_wired_v_read;
                                                    ...
                                                    // Issue #2911: unified refined-consistency hard
                                                    // gate.
                                                    insert_kv(
                                                        "refined-consistency-wired",
                                                        static_cast<std::int64_t>(
                                                            refined_consistency_wired_v_read()));
                                                    insert_kv(
                                                        "refined-consistency-observe-total",
                                                        static_cast<std::int64_t>(
                                                            refined_consistency_observe_total_v_read()));
                                                    insert_kv(
                                                        "refined-consistency-reject-total",
                                                        static_cast<std::int64_t>(
                                                            refined_consistency_reject_total_v_read()));
                                                    insert_kv(
                                                        "refined-consistency-recover-total",
                                                        static_cast<std::int64_t>(
                                                            refined_consistency_recover_total_v_read()));
                                                    insert_kv("refined-consistency-face",
                                                              refined_consistency_drift_face_hit()
                                                                  ? 1
                                                                  : 0);
                                                    insert_kv("commit-readiness-force-reason-"
                                                              "refined-drift",
                                                              15);
                                                    insert_kv("schema-2911",
                                                              kRefinedConsistencyGateIssue);
                                                    insert_kv("issue-2911",
                                                              kRefinedConsistencyGateIssue);
                                                }
                                                ...
                                                // Issue #2939: dep-closure reverify (O(affected) vs
                                                // clean scan).
                                                {
                                                    const std::int64_t cl_nodes =
                                                        m ? static_cast<std::int64_t>(
                                                                m->delta_reverify_closure_nodes_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t cl_edges =
                                                        m ? static_cast<std::int64_t>(
                                                                m->delta_reverify_closure_edges_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t cl_cap =
                                                        m ? static_cast<std::int64_t>(
                                                                m->delta_reverify_closure_cap_hit_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    insert_kv("delta-reverify-closure-nodes-total",
                                                              cl_nodes);
                                                    insert_kv("delta_reverify_closure_nodes_total",
                                                              cl_nodes);
                                                    insert_kv("delta-reverify-closure-edges-total",
                                                              cl_edges);
                                                    insert_kv("delta_reverify_closure_edges_total",
                                                              cl_edges);
                                                    insert_kv(
                                                        "delta-reverify-closure-cap-hit-total",
                                                        cl_cap);
                                                    insert_kv(
                                                        "delta_reverify_closure_cap_hit_total",
                                                        cl_cap);
                                                ...
                                                // Issue #2991: high-frequency mutate blame
                                                // completeness.
                                                insert_kv("schema-2991", 2991);
                                                insert_kv("issue-2991", 2991);
                                                // Issue #2992: non-strict ground-type Agent
                                                // feedback.
                                                insert_kv("schema-2992", 2992);
                                                insert_kv("issue-2992", 2992);
                                                // Issue #2993: type-check metrics tier (minimal
                                                // default).
                                                ...
                                                {
                                                    const auto tier = static_cast<std::int64_t>(
                                                        aura::compiler::typecheck_metrics_tier());
                                                    insert_kv("typecheck-metrics-tier", tier);
                                                    insert_kv(
                                                        "typecheck-metrics-full",
                                                ...
                                                insert_kv("gradual-permissiveness-wired", 1);
                                                {
                                                    const std::int64_t gw =
                                                        m ? static_cast<std::int64_t>(
                                                                m->gradual_ground_incompatible_warning_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    const std::int64_t ge =
                                                        m ? static_cast<std::int64_t>(
                                                                m->gradual_ground_incompatible_error_total
                                                                    .load(
                                                                        std::memory_order_relaxed))
                                                          : 0;
                                                    insert_kv("ground-incompatible-warning-total",
                                                              gw);
                                                    insert_kv("ground-incompatible-error-total",
                                                              ge);
                                                }
                                                insert_kv(
                                                    "coercion-blame-chain-complete-total",
                                                    static_cast<std::int64_t>(
                                                        aura::compiler::
                                                            g_coercion_blame_chain_complete_total
                                                                .load(std::memory_order_relaxed)));
                                                insert_kv(
                                                    "coercion-blame-missing-total",
                                                    static_cast<std::int64_t>(
                                                        aura::compiler::
                                                            g_coercion_blame_missing_total.load(
                                                                std::memory_order_relaxed)));
                                                insert_kv(
                                                    "coercion-blame-epoch-restamp-total",
                                                    static_cast<std::int64_t>(
                                                        aura::compiler::
                                                            g_coercion_blame_epoch_restamp_total
                                                                .load(std::memory_order_relaxed)));
                                                insert_kv(
                                                ...
                                                // commit_cs_has_work matrix.
                                                {
                                                    ...
                                                    // Issue #2898: explicit required TypeId
                                                    // invariant set on composite_txn_commit (anti
                                                    // under-mark false-green).
                                                    insert_kv(
                                                    ...
                                                    // Issue #2983: production default required
                                                    // TypeId auto-fill.
                                                    insert_kv(
                                                        ...
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                ...
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "composite-required-type-auto-fill-capped-"
                                                        "total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                ...
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "composite-required-type-reject-over-infer-"
                                                        "total",
                                                        static_cast<std::int64_t>(
                                                            g_typed_mutation_audit_counters
                                                                ...
                                                                .load(std::memory_order_relaxed)));
                                                    insert_kv(
                                                        "composite-required-type-auto-fill-wired",
                                                        1);
                                                    insert_kv(
                                                        "composite-required-type-auto-fill-cap",
                                                        static_cast<std::int64_t>(
                                                            aura::compiler::typed_audit::
                                                                kCompositeRequiredTypeAutoFillCap));
