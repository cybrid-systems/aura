    ...
    ObservabilityPrims::register_stats_impl(
        ...
        [&string_heap](std::span<const EvalValue> a) -> EvalValue {
            ...
            // above.
            {
                ...
                insert_kv("pending-full-solve-residual-escalate-"
                          "total",
                          static_cast<std::int64_t>(
                              aura::compiler::typed_audit::
                ...
                insert_kv("schema-3032",
                          aura::compiler::typed_audit::kRehydrateMissInvalidateIssue);
                insert_kv("issue-3032", aura::compiler::typed_audit::kRehydrateMissInvalidateIssue);
                // Issue #3063: steal/densify success invalidate.
                insert_kv(
            ...
            // + reverify roots.
            {
                ...
                const std::int64_t undermark_cs =
                    m ? static_cast<std::int64_t>(
                            m->adt_exhaust_undermark_force_total.load(std::memory_order_relaxed))
                      : 0;
                const std::int64_t undermark_dp = static_cast<std::int64_t>(
                    aura::compiler::dirty::adt_exhaust_undermark_force_total.load(
                        std::memory_order_relaxed));
                insert_kv("adt-exhaust-undermark-force-total", undermark_cs);
                insert_kv("adt-exhaust-undermark-force-dirty-total", undermark_dp);
            ...
            // rehydrate.
            {
                ...
                insert_kv("type-linear-proof-reject-empty-after-"
                          "fence-total",
                          static_cast<std::int64_t>(
                              aura::compiler::typed_audit::
                ...
                insert_kv("type-linear-proof-cleared-on-abort-"
                          "observe-total",
                          static_cast<std::int64_t>(
                              aura::compiler::typed_audit::
                ...
                // production default records.
                {
                    ...
                    // #2188 / #2347 counters.
                    {
                        ...
                        // / #2188 surfaces.
                        {
                            ...
                            // above.
                            insert_kv("region-type-cross-talk-"
                                      "observe-total",
                                      static_cast<std::int64_t>(
                                          aura::compiler::typed_audit::
                                              region_type_cross_talk_observe_total_v_read()));
                            insert_kv("region-type-cross-talk-reject-"
                                      "total",
                                      static_cast<std::int64_t>(
                                          aura::compiler::typed_audit::
                            ...
                            // preserved.
                            insert_kv("mutation-hold-budget-forced-"
                                      "fail-"
                                      "closed-total",
                                      static_cast<std::int64_t>(
            ...
            insert_kv("coercion-blame-hf-lag-wired",
                      static_cast<std::int64_t>(aura::compiler::g_coercion_blame_hf_lag_wired.load(
                          std::memory_order_relaxed)));
            insert_kv(
                "coercion-blame-session-force-total",
                static_cast<std::int64_t>(aura::compiler::g_coercion_blame_session_force_total.load(
                    std::memory_order_relaxed)));
            insert_kv("coercion-blame-stale-narrowing-drop-total",
                      static_cast<std::int64_t>(
                          aura::compiler::g_coercion_blame_stale_narrowing_drop_total.load(
                              std::memory_order_relaxed)));
                ...
                insert_kv(
                    ...
                    static_cast<std::int64_t>(
                        g_typed_mutation_audit_counters.composite_required_type_auto_fill_total
                            .load(std::memory_order_relaxed)));
                insert_kv("composite-required-type-auto-fill-capped-"
                          "total",
                          static_cast<std::int64_t>(
                              g_typed_mutation_audit_counters
                                  .composite_required_type_auto_fill_capped_total.load(
                                      std::memory_order_relaxed)));
                insert_kv("composite-required-type-reject-over-infer-"
                          "total",
                          static_cast<std::int64_t>(
                              g_typed_mutation_audit_counters
