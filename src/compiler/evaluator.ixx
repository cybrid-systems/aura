...
// via require_effect_for_node_id / require_effect_on_ref before Guard
// (no 2-arg default ref_tenant=0). Soft/Off short-circuit at the
// compile helper (zero extra stores). Coverage linter:
...
public:
    ...
    // Issue #1952 / #1930: AOT incremental re-emit stats (#1480 pipeline +
    // real LLVM emit + stable name→func_id map). Backed by CompilerMetrics
    // fields bumped from aura_jit_bridge.cpp (aura_reemit_aot_for_dirty).
    // Exposed via (engine:metrics "query:aot-incremental-reemit-stats").
    ...
    [[nodiscard]] std::uint64_t get_stable_func_id_assigned_total() const noexcept {
        if (auto* m = static_cast<CompilerMetrics*>(compiler_metrics_))
    ...
    // capture remount metrics (hit / miss). Distinct from the
    // #2233 epoch_restamp_total (which only stamps func_id) — this
    // pair measures the **capture state** rebind outcome.
    [[nodiscard]] std::uint64_t get_closure_capture_remount_ok_total() const noexcept {
        if (auto* m = static_cast<CompilerMetrics*>(compiler_metrics_))
            return m->closure_capture_remount_ok_total.load(std::memory_order_relaxed);
        return 0;
    }
    [[nodiscard]] std::uint64_t get_closure_capture_remount_fail_total() const noexcept {
        if (auto* m = static_cast<CompilerMetrics*>(compiler_metrics_))
    ...
    // Issue #2935 / #3055 / #3057: auto-register known intermediate +
    // compiler external root slots into the Moving densify window
    ...
    // aliases (#3057 — slot rewrite, not create-point observe). EnvFrame
    // / JIT residual after objects_moved is fail-closed via
    // last_object_remap_ stale scan (#3055) — not a second remap
    // registry.
...
private:
    ...
    // (contributing.md §3 — moved to source code; body in evaluator_adt.cpp).
...
public:
    ...
    // Issue #3000 / #3037 / #3230: production + restamp_over_budget_torn
    ...
    // is the post-mutate authority after over-budget. Never restore a
    // pre-mutate gen onto a remade layout (#3230).
    ...
    // Exit a mutation boundary. Pops the checkpoint. If success is true,
    // the version advance is kept; if false, mutations recorded between
...
#define AURA_MUTATION_BOUNDARY_PROTECT(EV, BODY)                                                   \
    ...
    // Wave 3a: MutationBoundaryGuard declaration stays nested (private
    ...
    public:
        ...
        // Template must stay in the interface (module BMI).
        template <typename F>
...
// #1868).
//
// Issue #1888 / #1926 / #1929 / #1954: `source_lifetime_version` + `live`
// enable explicit guards. After Closure move / GC erase /
// tombstone_for_views(), revalidate with is_closure_view_valid(view, live_cl)
// before any deref of flat/pool/params. make_closure_view rejects already-
// tombstoned sources (bumps closure_view_dangling_prevented_total).
// #1929 / #1954 close the hot-update Closure Bridge mandate: dual-epoch
// stamp + safe accessors + apply-path snapshot revalidate +
// walk_active_closures on mutation/GC/JIT/fiber bounds.
