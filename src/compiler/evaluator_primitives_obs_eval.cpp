...
void ObservabilityPrims::register_eval_p73(PrimRegistrar add, Evaluator& ev) {
    ...
    // primitives_style.md).
    ObservabilityPrims::register_stats_impl(
        "query:eda-production-readiness", [&ev](const auto&) -> EvalValue {
            ...
            auto milestone_pct = [&](std::initializer_list<const char*> expected) -> std::int64_t {
                ...
                return static_cast<std::int64_t>((found * ::aura::compiler::kBasisPointScale) /
                                                 total);
            };
...
void ObservabilityPrims::register_eval_p85(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:eda-foundation-stats", [&ev](const auto&) -> EvalValue {
            auto build_hash =
                [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
                auto* ht = FlatHashTable::create(16);
                if (!ht)
...
void ObservabilityPrims::register_eval_p87(PrimRegistrar add, Evaluator& ev) {
    ...
    //   - schema == 841
    ObservabilityPrims::register_stats_impl(
        "query:eda-infra-stats", [&ev](const auto&) -> EvalValue {
            auto build_hash =
                [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
                auto* ht = FlatHashTable::create(8);
                if (!ht)
