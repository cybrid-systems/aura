...
void register_security_primitives(PrimRegistrar add, Evaluator& ev) {
    ...
    add("query:primitives-registry-core-stats", [&ev](const auto&) -> EvalValue {
        auto build_hash = [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
            auto* ht = FlatHashTable::create(16);
            if (!ht)
                return make_void();
        ...
        const std::int64_t error_rate_pct = static_cast<std::int64_t>((errors * 100) / call_denom);
    ...
    add("query:primitives-ai-native-stats", [&ev](const auto&) -> EvalValue {
        auto build_hash = [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
            auto* ht = FlatHashTable::create(16);
            if (!ht)
                return make_void();
    ...
    add("query:stable-ref-sv-scale-stats", [&ev](const auto&) -> EvalValue {
        ...
        const std::uint64_t total = wrap_events + dirty_calls + dirty_nodes + max_depth +
                                    stale_ref + invalidations + sv_nodes + compact_trigger_count;
        std::int64_t recommendation = 0;
        ...
        std::vector<std::pair<std::string, EvalValue>> kv = {
            ...
            {"compact-trigger-count", make_int(static_cast<std::int64_t>(compact_trigger_count))},
            {"stale-ref-on-large-ast", make_int(static_cast<std::int64_t>(stale_ref_on_large_ast))},
    ...
    //   - gc-root-resync
    //   - schema == 740
    add("query:linear-jit-safety-stats", [&ev](const auto&) -> EvalValue {
        auto build_hash = [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
            auto* ht = FlatHashTable::create(8);
            if (!ht)
                return make_void();
