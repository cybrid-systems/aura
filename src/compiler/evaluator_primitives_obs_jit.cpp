...
void ObservabilityPrims::register_jit_p2(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:jit-hotswap-closure-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(12)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:pass-pipeline-stats", [&ev](const auto&) -> EvalValue {
            ...
            const std::uint64_t pipeline_total = pipeline_yield + passes_skip_dirty +
                                                 ...
                                                 mod_skip;
            auto* ht = FlatHashTable::create(query_hash_capacity_for(22)); // #2823 schema keys
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:pass-pipeline-dirtyaware-stats", [&ev](const auto&) -> EvalValue {
            auto build_hash =
                ...
                auto* ht = FlatHashTable::create(query_hash_capacity_for(39));
                if (!ht)
                    return make_void();
                bool overflowed = false;
                auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:soa-dirty-stats", [&ev](const auto&) -> EvalValue {
            ...
            std::vector<std::pair<std::string, EvalValue>> kv = {
                ...
                {"soa-dirty-desync-detected-total",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_detected_total))},
                {"soa_dirty_desync_force_full_total",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_force_full_total))},
                {"soa-dirty-desync-force-full-total",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_force_full_total))},
                {"soa_dirty_desync_synced_bits_total",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_synced_bits_total))},
                {"soa-dirty-desync-synced-bits-total",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_synced_bits_total))},
                {"soa-dirty-desync-gate-wired",
                 make_int(L(&CompilerMetrics::soa_dirty_desync_gate_wired))},
                {"schema-2181", make_int(2181)},
                {"issue-2181", make_int(2181)},
...
void ObservabilityPrims::register_jit_p10(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl("gc-arena-info", [&ev](const auto&) -> EvalValue {
        ...
        auto build_hash = [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
            auto* ht = FlatHashTable::create(query_hash_capacity_for(17)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:macro-hygiene-provenance-stats", [&ev](const auto&) -> EvalValue {
            auto build_hash =
                [&](std::span<const std::pair<std::string, EvalValue>> kv) -> EvalValue {
                auto* ht =
                    FlatHashTable::create(query_hash_capacity_for(30)); // #2037 mutate-hygiene keys
                if (!ht)
                    return make_void();
                bool overflowed = false;
                auto meta = ht->metadata();
...
void ObservabilityPrims::register_jit_p26(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:envframe-dualpath-mandatory-enforce-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(16)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
...
void ObservabilityPrims::register_jit_p33(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        ...
        [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(15)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
...
void ObservabilityPrims::register_jit_p39(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:error-handling-policy-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(13)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
...
void ObservabilityPrims::register_jit_p40(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:fiber-scheduler-init-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(14)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:typed-mutation-audit-stats", [&ev](const auto&) -> EvalValue {
            ...
            insert_kv("adt-exhaustiveness-audit-total",
                      static_cast<std::int64_t>(
                          g_typed_mutation_audit_counters.adt_exhaustiveness_audit_total.load(
                              std::memory_order_relaxed)));
            insert_kv("adt-exhaustiveness-fail-total",
                      static_cast<std::int64_t>(
                          g_typed_mutation_audit_counters.adt_exhaustiveness_fail_total.load(
                              std::memory_order_relaxed)));
            insert_kv("adt-exhaustiveness-hard-gate-wired",
                      static_cast<std::int64_t>(
                          g_typed_mutation_audit_counters.adt_exhaustiveness_hard_gate_wired.load(
                              std::memory_order_relaxed)));
...
void ObservabilityPrims::register_jit_p91(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:ownership-escape-postmutate-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht =
                FlatHashTable::create(query_hash_capacity_for(74)) /* #1141 / #2344 / #3085 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
...
void ObservabilityPrims::register_jit_p97(PrimRegistrar add, Evaluator& ev) {
    ...
    ObservabilityPrims::register_stats_impl(
        "query:resource-quota-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(
                query_hash_capacity_for(83)) /* #1141 / #1498 / #1590 / #1618 more keys */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:per-fiber-mutation-stack-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* m = static_cast<CompilerMetrics*>(ev.compiler_metrics());
            auto* ht = FlatHashTable::create(query_hash_capacity_for(19)) /* #1141 */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
        ...
        "query:gc-safepoint-adaptive-stats", [&ev](const auto&) -> EvalValue {
            ...
            const std::int64_t adapt_down =
                m ? static_cast<std::int64_t>(
                  ...
                  : 0;
            auto* ht =
                FlatHashTable::create(query_hash_capacity_for(20)) /* #1141 / #1599 hist buckets */;
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
    ...
    ObservabilityPrims::register_stats_impl(
        "query:gc-defer-reason-stats", [&ev](const auto&) -> EvalValue {
            ...
            auto* ht = FlatHashTable::create(query_hash_capacity_for(135));
            if (!ht)
                return make_void();
            bool overflowed = false;
            auto meta = ht->metadata();
