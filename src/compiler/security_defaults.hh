...
#include "compiler/lock_order_audit.h"     // Issue #2557 / #3119 production Hard lock-order audit
...
//  12. Lock-order audit (#2557 / #3119 / refine #2354):
//        - production Restricted/Strict → Hard (abort on inversion)
//        - AURA_SANDBOX=off → OFF (zero atomics; unit Soft path)
//        - AURA_LOCK_ORDER_CANARY=1 → hard abort (always wins)
//        - AURA_LOCK_ORDER_AUDIT=soft|1 → explicit Soft; 0|off → off
...
inline void apply_production_security_defaults() noexcept {
    ...
    // 13) Issue #2557 / #3119: production lock-order audit. Restricted/
    //     Strict → Hard (abort on inversion). AURA_SANDBOX=off → OFF.
    //     AURA_LOCK_ORDER_CANARY=1 still Hard. AUDIT=soft|1 opt-in Soft;
    //     AUDIT=0|off forces off.
