...
namespace {
    ...
    void check_call_in_func(const IRFunction& func, const IRModule& mod, const IRInstruction& instr,
        ...
        if (callee_id >= 0 && callee_id < static_cast<int>(mod.functions.size())) {
            ...
            if (callee.variadic) {
                ...
                if (actual >= fixed)
                    return; // OK: captures rest as list
            }
            if (actual != expected) {
                ArityDiagnostic d;
                d.func_id = func.id;
                d.block_id = block_id;
                d.instr_index = instr_index;
                d.expected = expected;
                d.actual = actual;
                d.function_name = callee.name;
                d.message = "arity mismatch: call to '" + callee.name + "' expects " +
                            std::to_string(expected) + " args but got " + std::to_string(actual) +
                            " (caller=" + func.name + ")";
                diags.push_back(d);
            }
        }
        ...
        for (std::size_t bi = 0; bi < func.blocks.size(); ++bi) {
            ...
            for (std::size_t ii = 0; ii < block.instructions.size(); ++ii) {
                ...
                if (instr.opcode == IROpcode::Call) {
                    check_call_in_func(func, mod, instr, static_cast<std::uint32_t>(bi),
                                       static_cast<std::uint32_t>(ii), result.diagnostics);
                }
            }
        }
    }
