                ...
                if (do_audit) {
                    ...
                    if (!composite && !inv_ok && hard_gate) {
                        ...
                        if (!first.linear_ok || first.cross_batch_linear_escape) {
                            ...
                            // (single source of truth — densify + steal
                            // share the same helper per AC4). AC3 zero-cost
...
//
//   Issue #2990 ConcurrentMutationPolicy (workspace EDSL layer):
...
Evaluator::MutationBoundaryGuard::try_acquire(Evaluator& ev, std::uint64_t pending_count,
    ...
    // Issue #2746 / #2990: parallel-intend task body may have stamped a
    // region_key TLS. Redirect to try_acquire_for_region only when
    // ConcurrentMutationPolicy::ScopedParallel is opted in (production
    // default SingleWriter stays GlobalExclusive — AC1 zero regression).
    // Zero key / region machinery OFF / SingleWriter → GlobalExclusive.
    {
        const auto rk = Evaluator::parallel_task_region_key();
        if (rk != 0 && ev.workspace_region_concurrency_enabled() && ev.scoped_parallel_enabled()) {
            ev.bump_scoped_parallel_redirect();
            return try_acquire_for_region(ev, rk, pending_count, success_flag, fine_rollback);
        }
        if (rk != 0 && !ev.scoped_parallel_enabled())
            ev.bump_single_writer_serialize();
    }
...
Evaluator::MutationBoundaryGuard::try_acquire_for_region(Evaluator& ev, std::uint64_t region_key,
    ...
    // #2760: effective_region_cone_mask (TLS proven / key-derived).
    // #2761 residual: when both masks proven, mask-AND is sole authority
    // — unequal keys with overlapping cones REJECT (commercial multi-
    // hypothesis isolation). Missing mask → #2724 key fallback (no tree
    // walk). Soft stays metric-only (#2724 AC3 / #2754 AC3 / #2757 AC3 /
    // #2760 AC2 / #2761 AC3).
    //
    // Gate order preserved: mailbox → hold-budget → security-schedule →
    ...
    if (region_or_mask && typed_audit::production_defaults_active()) {
        ...
        // #2761 AC2: unequal keys + proven-disjoint masks → concurrent admit.
        ...
        if (regions_mask_disjoint(region_key, g_last_admitted_region_key, cone_mask,
                                  g_last_admitted_cone_mask)) {
        ...
        if (have_prior_soft && !regions_disjoint(region_key, g_last_admitted_region_key_soft,
            ...
            if (regions_cone_disjoint(region_key, g_last_admitted_region_key_soft, cone_mask,
                                      g_last_admitted_cone_mask_soft)) {
                g_mutation_region_concurrent_cone_admit_total.fetch_add(1,
                                                                        std::memory_order_relaxed);
            }
            if (regions_mask_disjoint(region_key, g_last_admitted_region_key_soft, cone_mask,
                                      g_last_admitted_cone_mask_soft)) {
                g_mutation_region_mask_disjoint_admit_total.fetch_add(1, std::memory_order_relaxed);
            }
            if (cone_mask != 0)
    ...
    if (ev.workspace_flat_ && ev.workspace_flat_->atomic_batch_active()) {
        ...
        if (auto* m = static_cast<CompilerMetrics*>(ev.compiler_metrics_))
            m->workspace_region_fallback_global_total.fetch_add(1, std::memory_order_relaxed);
    } else if (scoped_parallel_overlap_fallback) {
        // Issue #2990: overlap under ScopedParallel → SingleWriter.
        key = std::nullopt;
        if (auto* m = static_cast<CompilerMetrics*>(ev.compiler_metrics_))
            m->workspace_region_fallback_global_total.fetch_add(1, std::memory_order_relaxed);
