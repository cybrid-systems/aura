...
// dirty cone (dirty_propagation + evaluator_typecheck + mutate_type_gate).
// Production hard-rejects non-exhaustive after recheck; Soft observe-only;
...
public:
    ...
    [[nodiscard]] SolverBudget solver_budget() const noexcept { return solver_budget_; }
    ...
    // Issue #3045: under-mark force — insert match NodeIds into reverify
    // roots when the Agent dirtied only a constructor / single arm.
    // Empty input → 0 (Quiet).
    [[nodiscard]] std::size_t
...
export struct TypeChecker {
    ...
    //       (including empty cone — columnar under-mark)
    ...
    // Issue #3545: persist-reject undo of apply_coercion_map mark_eliminated.
    void unmark_eliminated_recent(std::size_t n) noexcept { last_coercions_.unmark_eliminated(n); }
