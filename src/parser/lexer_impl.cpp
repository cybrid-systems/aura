module;
#include <cstddef>
#include <cctype>
#include <cstdint>
#include <string_view>
#include <string>

module aura.parser.lexer;

namespace aura::parser {

Token Lexer::advance() {
    skip_whitespace();
    if (pos_ >= source_.size())
        return make_token(TokenKind::EndOfFile, "");

    char c = source_[pos_];
    switch (c) {
    case '(': return make_token(TokenKind::LParen, source_.substr(pos_++, 1));
    case ')': return make_token(TokenKind::RParen, source_.substr(pos_++, 1));
    default:
        if (std::isdigit(static_cast<unsigned char>(c)) ||
            (c == '-' && pos_ + 1 < source_.size() &&
             std::isdigit(static_cast<unsigned char>(source_[pos_ + 1])))) {
            return read_number();
        }
        if (std::isalpha(static_cast<unsigned char>(c)) || c == '_' || c == '+' || c == '*')
            return read_identifier();
        return make_token(TokenKind::Error, source_.substr(pos_++, 1));
    }
}

Token Lexer::read_number() {
    size_t start = pos_;
    if (source_[pos_] == '-') pos_++;
    while (pos_ < source_.size() && std::isdigit(static_cast<unsigned char>(source_[pos_])))
        pos_++;
    return make_token(TokenKind::Integer, source_.substr(start, pos_ - start));
}

Token Lexer::read_identifier() {
    size_t start = pos_;
    while (pos_ < source_.size()) {
        char c = source_[pos_];
        if (std::isalnum(static_cast<unsigned char>(c)) || c == '_' ||
            c == '+' || c == '*' || c == '-' || c == '/' || c == '=' || c == '<' || c == '>' || c == '!')
            pos_++;
        else break;
    }
    return make_token(TokenKind::Identifier, source_.substr(start, pos_ - start));
}

void Lexer::skip_whitespace() {
    while (pos_ < source_.size()) {
        char c = source_[pos_];
        if (c == ' ' || c == '\t') {
            pos_++; col_++;
        } else if (c == '\n') {
            pos_++; line_++; col_ = 1;
        } else if (c == ';') {
            while (pos_ < source_.size() && source_[pos_] != '\n') pos_++;
        } else break;
    }
}

Token Lexer::peek() {
    if (!peeked_) {
        peeked_ = true;
        peek_token_ = advance();
    }
    return peek_token_;
}

Token Lexer::consume() {
    if (peeked_) {
        peeked_ = false;
        Token t = peek_token_;
        peek_token_ = {};
        return t;
    }
    return advance();
}

Token Lexer::make_token(TokenKind kind, std::string_view text) {
    Token t{kind, text, line_, col_};
    col_ += static_cast<uint32_t>(text.size());
    return t;
}

} // namespace aura::parser
