...
Token Lexer::read_identifier() { std::size_t s = pos_; while (pos_ < source_.size()) { char c = source_[pos_]; if (std::isalnum((unsigned char)c) || c == '_' || c == '+' || c == '*' || c == '-' || c == '/' || c == '=' || c == '<' || c == '>' || c == '!') pos_++; else break; } return make_tok(TokenKind::Identifier, source_.substr(s, pos_-s)); }
