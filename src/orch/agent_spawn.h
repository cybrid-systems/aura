...
struct OrchModuleStats {
    ...
    // by the parent, parent.cancel_all propagates top-down.
    std::atomic<std::uint64_t> scope_child_total{0}; // #2631
...
// Issue #3529: production reclaim-stuck timeout. Soft callers must not
// reach getenv (zero extra). Default 60s; AURA_RECLAIMED_QUOTA_TIMEOUT_MS
// overrides (0 = immediately eligible, soak/unit).
[[nodiscard]] inline std::uint64_t reclaimed_quota_timeout_ms() noexcept {
    const char* e = std::getenv("AURA_RECLAIMED_QUOTA_TIMEOUT_MS");
    if (!e || *e == '\0')
        return kReclaimedQuotaTimeoutMsDefault;
    char* end = nullptr;
...
[[nodiscard]] inline std::uint64_t reclaimed_age_ms(const serve::Fiber* f) noexcept {
    if (!f)
        return 0;
    const auto start = f->mark_reclaimed_steady_clock_ns();
    if (start <= 0)
        return 0;
    ...
    if (now <= start)
        return 0;
    return static_cast<std::uint64_t>(now - start) / 1000000ULL;
}

[[nodiscard]] inline bool reclaimed_quota_stuck_past_timeout(const serve::Fiber* f) noexcept {
...
// (orch:agent-send) auto-calls this after handoff. Issue #3013 / #3212:
// raw agent_send and direct mailbox->push both return HandoffRequired
// for unstamped held_ref_token (mailbox still bumps handoff_reject_total
// — defense in depth, never weakens the mailbox gate). Closed is
// true-closed / linear-viol only.
...
//     agent_send. Issue #3013 / #3212: raw agent_send and mailbox
//     push/broadcast with un-stamped held_ref_token both return
//     HandoffRequired (same typed fail as this helper) instead of
//     ambiguous Closed. Mailbox gate still rejects (#2663 defense
//     in depth); status is now aligned.
//
// Caller rules:
