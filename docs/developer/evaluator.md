...
**P0 refactor start (from Task 4 review)**: EnvFrame SoA migration (Issue #145). Step 1: removed raw `cells_` pointer from `EnvFrame` struct (now pure data: parent_id + bindings only). lookup_local* now return sentinels as-is; cell deref centralized in `Evaluator::lookup_by_symid_chain` using the single owning `cells_` pmr vector. Updated alloc comments + chain lookup. Legacy Env + its cells_ pointer still coexist (subsequent small steps will migrate call sites, remove parent_ pointers, centralize everything). See evaluator.ixx:215 (EnvFrame), evaluator_impl.cpp around alloc/lookup_by_symid_chain, and the review report §6 P0. Living docs updated same commit. Goal: cache-friendly index-only envs, no pointer chasing to re-allocatable heaps, full GC safety for closures.
