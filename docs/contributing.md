...
无状态原语可放入 `evaluator_primitives_*.cpp`（P0：`evaluator_primitives_core.cpp`），经 `PrimRegistrar` 回调注册。需访问 `pairs_` / `string_heap_` 的见 `evaluator_primitives_pair.cpp`；list 高阶原语（`map` / `filter` / `foldl`）见 `evaluator_primitives_list.cpp`（额外传入 `apply_unary` / `apply_pred` / `apply_binary` 回调）；JSON 见 `evaluator_primitives_json.cpp`；vector/hash 见 `evaluator_primitives_vector.cpp`（额外传入 `vector_heap_`）。
