...
**设计理念**：每个语言构造都是 Trees that Grow 的一个 Phase，必须穿透全部 7 层。

> ⚠️ Racket frontend and ABF pipeline removed 2026-05-14. Trees-That-Grow now uses FlatAST SoA directly + ABF v2 column-wise cache.
...
```
    ...
    ┌──────────────────┐  ┌────────────────┐  ┌──────────────────┐
    │  (ABF 已移除)     │  │                │  │                  │
    │  FlatAST SoA 直   │  │  5. Lowering   │  │  6. Reconstruct  │
    │  接输入求值器     │  │  lowering_impl │  │  low_flat_impl   │
    │  (无 .rkt 环节)   │  │  IR 指令生成    │  │  FlatAST→Expr*   │
    └──────────────────┘  └────────────────┘  └──────────────────┘
              │
              ▼
    ┌──────────────────┐
...
### Layer 5: Lowering (`lowering_impl.cpp`)
...
### Layer 6: Reconstruct (`lowering_flat_impl.cpp`)
...
### Layer 7: 查询引擎 (`query.ixx` + `query_impl.cpp`)
...
───────────────────────────────────────────
累计            25 min  28 min  15 min  10 min
```
...
1. **不要走捷径** — 每个新构造必须穿透所有 7 层（AST/SoA/解析器/求值器/Lowering/Reconstruct/查询引擎）。跳过一层就是技术债。
