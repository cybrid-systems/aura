...
（pilot-37 note）test_issue_169（Fine-grained Incremental Compilation v3 / config flag 验证）已转换。详见 evaluator.md §12。
...
（pilot-73 prep note）test_issue_126（pure fns + ir_cache_pure 验证）。Short 尝试 hit module; full 恢复。历史 passes。详见 evaluator.md §12。继续。
...
（pilot-76 prep note）test_issue_130（cache hit rate 验证）。Short 尝试 hit module dep; full 恢复健康。历史 counts。详见 evaluator.md §12。继续。
